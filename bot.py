# bot.py - Full fixed RazorPay API (parses status/message from new API format)
...
# ✅ RAZORPAY API URL
RAZORPAY_API_URL = os.getenv("RAZORPAY_API_URL", "https://ravenkiller.site/rz/r.php")
...
def is_site_truly_dead(response, price):
    ...
    dead_indicators = [
        ...
        'cannot ship to this address', 'delivery_delivery_line_detail_changed', 'delivery_address2_required',
        'delivery_line_detail_changed', 'delivery_line', 'delivery_address', 'address_required',
        'submit_rejected', 'shipping method not available', 'site error! status: 400', 'site error! status: 401',
        ...
        'api_timeout', 'tlsv1 alert', 'ssl routines', 'openssl ssl_connect', 'ssl certificate error',
        'ssl handshake failed', 'ssl error', 'tls connection failed', 'certificate verify failed',
        'self signed certificate', 'expired certificate', 'remote host closed connection', 'broken pipe',
        'dns lookup failed', 'dns resolution failed', 'dns error', 'cannot resolve host', 'hostname not found',
        'name resolution failed', 'getaddrinfo failed', 'dns server not responding', 'rate limit exceeded',
        'too many requests', 'request was throttled', 'detected unusual activity', 'bot protection enabled',
        'captcha required', 'cloudflare', 'access denied', 'blocked by cloudflare', 'ddos protection',
        'just a moment', 'ray id:', 'cf-ray:', 'invalid json', 'json decode error', 'json parse error',
        'site error', 'site dead', 'server error', 'internal_server_error', 'unexpected error',
        'something went wrong', 'nonetype', 'unknown error', 'unable to process', 'payment provider error',
        'payment gateway error', 'checkout_expired', 'receipt_empty', 'receipt id is empty',
        'site requires login', 'failed to get checkout', 'error processing card', 'decision_rule_block',
        'validation_custom', 'invalid_payment_method',
    ]
...
def classify_rz_response(rj, raw_text=None):
    gate = 'RazorPay'
    # First try to get message from JSON, else use raw_text
    resp = ""
    status_str = ""
    if isinstance(rj, dict):
        # New API format: {"status":"dead","message":"Payment declined ..."}
        status_str = str(rj.get('status', '')).lower()
        resp = str(rj.get('message', rj.get('response', rj.get('Response', ''))))
    ...
    # Map API status field to bot status
    if status_str in ('live', 'charged', 'success', 'captured', 'authorized'):
        return {"Response": resp, "Price": "-", "Gateway": gate, "Status": "Charged"}
    if status_str in ('dead', 'declined', 'fail', 'failed', 'error'):
        return {"Response": resp, "Price": "-", "Gateway": gate, "Status": "Declined"}

    # Fallback to keyword matching if status field is missing
    rz_charged = [
    ...
    rz_declined = [
        ...
        'payment declined', 'payment_risk_check_failed'
    ]
