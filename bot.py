```python
# bot.py - Fully Updated & Fixed (No Proxy Issues, RazorPay API Fixed)
...
async def check_rz_api(card, proxy_data=None, user_id=None, http_session=None):
    ...
    try:
        ...
        if "proxy" in err_msg.lower() or "connection" in err_msg.lower():
            return {"Response": "Proxy connection failed - API may be blocking proxy", "Price": "-", "Gateway": "RazorPay", "Status": "Error", "card": card}
        return {"Response": err_msg[:100], "Price": "-", "Gateway": "RazorPay", "Status": "RetryError", "card": card}
...
async def check_rz_with_retry(card, proxies_data=None, user_id=None, max_retries=3, cancel_check=None, http_session=None):
    ...
    for attempt in range(max_retries):
        ...
        proxy_data = None
        if proxies_data and len(proxies_data) > 0:
            available_px = [p for p in proxies_data if p.get('proxy_url') not in tried_proxies] or list(proxies_data)
            if available_px:
                proxy_data = random.choice(available_px)
                if proxy_data: 
