...
from database import (
    ...
    is_banned_user, ban_user, unban_user, get_user_expiry,
    ...
    add_site_db, get_user_sites, get_user_sites_with_info, remove_site_db, update_site_info,
    save_card_to_db, get_total_cards_count, get_charged_count, get_approved_count, get_cards_by_status,
    get_all_premium_users, get_total_users, get_premium_count,
    get_total_sites_count, get_users_with_sites, get_sites_per_user, get_all_sites_detail, get_daily_stats,
    ...
    get_all_active_codes, get_all_codes, get_code_usage_stats,
    get_mongo_db, is_mongo_healthy, get_db_status, backup_to_json, restore_from_json
)
...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8981624250:AAH0t3mIOVKmLItZxHlT85niCkdmWh1i_TM")
ADMIN_ID = json.loads(os.getenv("ADMIN_ID", "[7935621079]"))
HIT_CHANNEL_ID = int(os.getenv("HIT_CHANNEL_ID", "-1001234567890"))
JOIN_GROUP_ID = int(os.getenv("JOIN_GROUP_ID", "-1001234567890"))
JOIN_CHANNEL_ID = int(os.getenv("JOIN_CHANNEL_ID", "-1001234567890"))
JOIN_GROUP_LINK = os.getenv("JOIN_GROUP_LINK", "https://t.me/your_group")
JOIN_CHANNEL_LINK = os.getenv("JOIN_CHANNEL_LINK", "https://t.me/your_channel")
FORCE_JOIN_IMAGES = [
    "",
    ""
]
...
SITE_ERROR_KEYWORDS = [
    ...
    'product id is empty', 'py id empty', 'clinte token', 'receipt_empty',
    'receipt id is empty', 'receipt empty', 'site requires login',
    'failed to get token', 'no valid products', 'not shopify',
    ...
    'no product found', 'checkout is not available', 'cart is empty',
    'cart add failed after retries', 'checkout_expired', 'checkout_not_found',
    'no shipping methods available', 'payment method not available',
    'Payment method not available', 'no valid payment method found',
    'No valid payment method found', 'payment method not found',
    'payment method unavailable', 'payment method not supported',
    'no payment methods available', 'delivery_no_delivery_strategy',
    'DELIVERY_NO_DELIVERY_STRATEGY', 'delivery_strategy_conditions',
    'DELIVERY_STRATEGY_CONDITIONS', 'delivery_strategy_conditions_1',
    'DELIVERY_STRATEGY_CONDITIONS_1', 'no shipping methods available',
    ...
    'checkout is disabled', 'checkout not available for this store',
    'checkout is locked', 'checkout limit reached', 'checkout is not active',
    'cart could not be created', 'checkout token expired', 'checkout session expired',
    'checkout is already completed', 'checkout is already paid',
    'shopify checkouts are unavailable', 'checkout not found', 'checkout expired',
    'checkout token empty', 'failed to get checkout url', 'checkout creation failed',
    'checkout id is empty', 'checkout is null', 'shop is currently unavailable',
    'store is currently unavailable', 'this shop is currently unavailable',
    'this store is currently unavailable', 'shop is unavailable', 'store is unavailable',
    'shop closed', 'store closed', 'this store is private', 'store not found',
    'page not found', 'store is password protected', 'enter store using password',
    'storefront is password protected', 'password protected', 'closed for maintenance',
    'under maintenance', 'site error! status: 400', 'site error! status: 401',
    'site error! status: 403', 'site error! status: 404', 'site error! status: 408',
    'site error! status: 429', 'site error! status: 500', 'site error! status: 501',
    'site error! status: 502', 'site error! status: 503', 'site error! status: 504',
    'site error! status: 505', 'site error! status: 520', 'site error! status: 521',
    'site error! status: 522', 'site error! status: 523', 'site error! status: 524',
    'site error! status: 525', 'site error! status: 526', 'site error! status: 527',
    'site error! status: 530', 'http 400', 'http 401', 'http 403', 'http 404',
    'http 408', 'http 429', 'http 500', 'http 502', 'http 503', 'http 504',
    'http 520', 'http 521', 'http 522', 'http 523', 'http 524', 'http 525',
    ...
    'amount_too_small', 'amount too small', 'product out of stock',
    ...
    'max redirects exceeded', 'infinite redirect', "'products'", "error:",
    "error: '", 'cart add failed', 'cart is empty', "'errors'", '"errors"',
    '{"errors":', 'errors":{', 'unauthenticated', 'unauthorized', 'invalid api key',
    'access token invalid', 'throttled', 'shop is frozen', 'store is frozen',
    'shop has been frozen', 'api limit exceeded', 'quota exceeded',
    'usage limit reached', 'request too large', 'payload too large',
    'entity too large', 'failed after retries', 'max retries exceeded',
    'retry limit exceeded', 'temporary failure', 'try again later',
    'please try again', 'internal error', 'system error', 'fatal error',
    'critical error', 'ARTIFACT_DISSATISFACTION', 'artifact_dissatisfaction',
    'TAX_NEW_TAX_MUST_BE_ACCEPTED', 'tax_new_tax_must_be_accepted',
    'PROCESSING_ERROR', 'processing_error', 'DECISION_RULE_BLOCK',
    'decision_rule_block', 'unable to get payment token', 'empty submit response',
    'empty submit', 'order_total_changed', 'order total changed',
    'invalid_payment_method', 'invalid payment method', 'validation_custom',
    'validation custom', 'payment provider error', 'payment gateway error',
    'payment processor error', 'error processing card', 'we could not process',
    'unable to process',
]
...
def is_site_truly_dead(response, price):
    ...
    exact_dead = [
        'MERCHANDISE_PRODUCT_NOT_PUBLIC', 'PAYMENTS_INVALID_GATEWAY_FOR_I',
        'PAYMENTS_INVALID_GATEWAY', 'CHECKOUT_NOT_FOUND', 'PAGE_NOT_FOUND',
        'STORE_NOT_FOUND', 'PRODUCT_NOT_FOUND', 'NO_PRODUCT_FOUND',
        'CART_IS_EMPTY', 'CHECKOUT_DISABLED', 'MERCHANDISE_NOT_ENOUGH_STOCK',
    ]
    ...
    dead_indicators = [
        ...
        'checkout is disabled', 'checkout not available',
        'checkout not available for this store', 'checkout is locked',
        'checkout limit reached', 'checkout is not active', 'checkout not found',
        'checkout expired', 'checkout token empty', 'checkout token not found',
        'checkout token is empty', 'checkout is already completed',
        'checkout is already paid', 'checkout creation failed',
        'checkout id is empty', 'checkout is null', 'cart is empty',
        'cart could not be created', 'cart add failed after retries',
        'cart add failed', 'no product found', 'product out of stock',
        'product not found', 'no valid products', 'product id is empty',
        'r2 id empty', 'py id empty', 'failed to get token', 'token not found',
        'no_session_token', 'no session token', 'no checkout token found',
        ...
        'this store is currently unavailable', 'store is unavailable',
        'shop is currently unavailable', 'this shop is currently unavailable',
        'shop is unavailable', 'shop closed', 'store closed', 'this store is private',
        'store not found', 'page not found', 'store is password protected',
        'enter store using password', 'storefront is password protected',
        'password protected', 'closed for maintenance', 'under maintenance',
        'shop is frozen', 'store is frozen', 'shop has been frozen',
        'delivery_no_delivery_strategy', 'delivery_strategy_conditions',
        'delivery_strategy_conditions_1', 'no shipping methods available',
        'delivery not available', 'delivery_company_required',
        'delivery_address_required', 'cannot ship to this address',
        'delivery_delivery_line_detail_changed', 'delivery_address2_required',
        'delivery_line_detail_changed', 'delivery_line', 'delivery_address',
        'address_required', 'submit_rejected', 'shipping method not available',
        ...
        'just a moment', 'ray id:', 'cf-ray:', 'invalid json', 'json decode error',
        'json parse error', 'site error', 'site dead', 'server error',
        'internal_server_error', 'unexpected error', 'something went wrong',
        'nonetype', 'unknown error', 'unable to process', 'payment provider error',
        'payment gateway error', 'checkout_expired', 'receipt_empty',
        'receipt id is empty', 'site requires login', 'failed to get checkout',
    ]
...
┗ ⚡ <i>{bs('Check the code and try again')}</i>""", emoji_ids=[CE["cross"]])
    ...
    if is_paid_plan(current_plan):
        expiry = await get_user_expiry(uid)
        expiry_str = expiry[:10] if expiry else "Unknown"
    ...
    async def worker(card):
        ...
        async with user_sem:
            ...
            try:
                ...
                detail = {
                    "card": card, "status": status, "gateway": gw,
                    "response": resp, "site": site, "price": price,
                    "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "result_type": status.upper()
                }
                ...
                if not is_stopped(): 
                    ...
                    detailed_results.append({
                        "card": card, "status": "Error", "gateway": gate_name,
                        "response": str(e)[:100], "site": "N/A", "price": "-",
                        "result_type": "ERROR", "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    })
...
async def send_detailed_results_file(uid, charged, approved, declined, errors, total, detailed_results, gate_name):
    ...
    try:
        async with aiofiles.open(fn, 'w', encoding='utf-8') as f:
            ...
            await f.write(f"\n{'='*70}\n\n")
            await f.write(f"📝 DETAILED RESULTS\n")
            await f.write(f"{'-'*70}\n\n")
...
async def planall_cmd(event):
    if event.sender_id not in ADMIN_ID: 
        return
    all_users = await get_all_premium_users()
    if not all_users: 
...
async def stats_cmd(event):
    ...
    try:
        ...
        code_stats = await get_code_usage_stats()
...
<b>━━━━━━━━━━━━━━━━━</b>
{PE} <b>{bs('Today')}:</b> <code>{daily_stats.get('cards', 0)}</code> {bs('checks')} | <code>{daily_stats.get('charged', 0)}</code> {bs('charged')}
<b>━━━━━━━━━━━━━━━━━</b>
...
# ====================== DB STATUS COMMAND ======================
@client.on(events.NewMessage(pattern=r'(?i)^[/.]dbstatus$'))
async def db_status_cmd(event):
    if event.sender_id not in ADMIN_ID:
        return
    status = await get_db_status()
    mongo_healthy = await is_mongo_healthy()
    await styled_reply(event, f"""📊 <b>{bs('Database Status')}</b>
━━━━━━━━━━━━━━━━━
MongoDB: {'✅ Connected' if mongo_healthy else '❌ Not Connected'}
JSON Backup: {'✅ Available' if status['json'] else '❌ Not Available'}
Active Storage: <code>{status['active_storage']}</code>""", emoji_ids=[CE["info"]])
...
async def main():
    ...
    status = await get_db_status()
    if status['mongo']:
        log_system("MONGODB", "✅ MongoDB connected successfully!")
    else:
        log_system("MONGODB", "⚠️ MongoDB not connected, using JSON fallback")
    ...
    while True:
        try:
            ...
            log_system("CONNECTION", f"Connection lost: {e}", "warning")
