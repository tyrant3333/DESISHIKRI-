...
# Try to import motor, if fails use JSON only
try:
...
DB_FILE = os.path.join(DB_DIR, "razor_bot_data.json")
...
_json_cache = None
_json_cache_time = 0
_JSON_CACHE_TTL = 5  # seconds

# ============ JSON STORAGE (Primary for Railway) ============
...
async def get_mongo_db():
    """Get MongoDB connection - safe with error handling"""
    ...
    if _mongo_client is None:
        try:
            ...
            print("⚠️ Falling back to JSON storage for this session")
            _mongo_client = None
            _db = None
            return None
    return _db
...
async def get_user_sites_with_info(user_id: int) -> List[Dict]:
    """Get all sites with info for user"""
    if USE_MONGODB and MOTOR_AVAILABLE:
        db = await get_mongo_db()
        if db:
            try:
...
async def generate_plan_code(plan_key: str, count: int = 1) -> List[str]:
    """Generate plan redemption codes"""
...
async def get_total_sites_count() -> int:
    """Get total sites count across all users"""
    if USE_MONGODB and MOTOR_AVAILABLE:
        db = await get_mongo_db()
        if db:
            try:
...
async def init_db():
    """Initialize database - always works, no connection errors"""
    print("=" * 50)
