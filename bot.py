# bot.py – Full updated with global sites + speed optimisations (retries reduced, connect timeout lowered, BIN cache)
...
log.addHandler(_ch)
try:
    _fh = logging.FileHandler('EnVoXChkBoTbot.log', encoding='utf-8')
...
# ── Timeout Configuration (keep API_TIMEOUT at 60 as requested) ──
API_TIMEOUT = 60
BIN_TIMEOUT = 10          # BIN lookup is fast; reduced from 60
PROXY_TIMEOUT = 12
RZ_TIMEOUT = 60
...
# ── BIN Cache ──
_BIN_CACHE = {}
_BIN_CACHE_TTL = 3600  # 1 hour


def get_bin_cache_key(bin6):
    return bin6


def get_cached_bin(bin6):
    entry = _BIN_CACHE.get(bin6)
    if entry and time.time() - entry["time"] < _BIN_CACHE_TTL:
        return entry["data"]
    return None


def set_cached_bin(bin6, data):
    _BIN_CACHE[bin6] = {"data": data, "time": time.time()}
...
HIT_BUTTON = [[Button.url(bs("𝙴𝚗𝚅𝚘𝚇 𝙲𝚑𝚔"), "https://t.me/EnVoXChkBoT")]]
...
async def get_user_http_session(uid, purpose="general"):
    ...
    if session is None or session.closed:
        ...
        # Lower connect timeout to 5s for faster fail on unreachable proxies
        session = aiohttp.ClientSession(
            timeout=aiohttp.ClientTimeout(total=timeout_val, connect=5),
            connector=connector,
        )
...
SITE_ERROR_KEYWORDS = [
    ...
    # === NEW SHOPIFY ERRORS (ADDED FROM RESULT FILE) ===
    'NO_PRODUCT',
...
def is_site_truly_dead(response, price):
    ...
    dead_indicators = [
        ...
        'price_over_max', 'login_in_site_require', 'authentication_failed',
    ]
    ...
    # Try to parse price – if price is 0, treat as dead (to avoid adding useless sites)
    try:
    ...
    # If price is 0, the site is not useful – mark as dead
    if pv == 0:
        return True
...
def is_truly_alive(response, price):
    ...
    # If price is 0, it's not alive (already handled in is_site_truly_dead)
    if pv == 0.0:
        return False
    ...
    try:
        ...
        async with _BIN_SEM:
            async with s.get(f'https://bins.antipublic.cc/bins/{bin6}') as r:
                if r.status != 200:
                    data = {"brand": "-", "type": "-", "level": "-", "bank": "-", "country": "-", "flag": "🏳️"}
                    set_cached_bin(bin6, data)
                    return data
                d = await r.json(content_type=None)
                data = {"brand": d.get('brand', '-'), "type": d.get('type', '-'), "level": d.get('level', '-'), "bank": d.get('bank', '-'), "country": d.get('country_name', '-'), "flag": d.get('country_flag', '🏳️')}
                set_cached_bin(bin6, data)
                return data
    except:
        data = {"brand": "-", "type": "-", "level": "-", "bank": "-", "country": "-", "flag": "🏳️"}
...
def classify_response(rj):
    ...
    # --- NEW: call is_site_truly_dead here to catch all dead site patterns ---
    if is_site_error(ar) or is_proxy_error(ar) or is_site_truly_dead(ar, price):
    ...
    declined = [
        ...
        'stolen_card', 'stolen card', 'lost_card', 'lost card', 'pickup_card',
        'pick_up_card', 'restricted_card', 'restricted card', 'fraudulent',
        'fraud suspected', 'fraud_suspected', 'expired_card', 'expired card',
...
async def check_card_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=3, rotator=None, cancel_check=None, http_session=None):
    ...
    for attempt in range(max_retries):
        ...
        if attempt < max_retries - 1: 
            await asyncio.sleep(0.1)  # reduced from 0.3 to 0.1
    if last:
        last["Status"] = "Error"
        return last, -1
...
# ====================== CLIENT ======================
client = TelegramClient('EnVoXChkBoTbot', API_ID, API_HASH)
...
async def info_cmd(event):
    ...
    await ensure_user(event.sender_id)
    plan = await get_user_plan(event.sender_id)
    global_sites = await get_global_sites_with_info()
...
# ====================== UPDATED /site COMMAND (uses is_site_truly_dead already) ======================
...
async def single_cc_check(event):
    ...
    st = time.time()
    rotator = SmartRotator(user_id=uid)
    try:
        ...
        async with get_user_sem(uid, "sp"):
            ...
            # Reduce max_retries to 2 for speed
            result, _ = await check_card_with_retry(card, sites, uid, proxies, max_retries=2, rotator=rotator, http_session=http_session)
            bi = await bin_task
...
async def rz_single_check(event):
    ...
    try:
        ...
        bin_task = asyncio.create_task(get_bin_info(card.split('|')[0]))
        # Reduce max_retries to 2 for speed
        result = await check_rz_with_retry(card, proxies, uid, max_retries=2, http_session=http_session)
        bi = await bin_task
        elapsed = round(time.time() - st, 2)
        status = result.get('Status', 'Declined')
        if status in ["Charged", "Approved"]:
...
async def send_detailed_results_file(uid, charged, approved, declined, errors, total, detailed_results, gate_name):
    fn = f"𝙴𝚗𝚅𝚘𝚇_𝙲𝚑𝚔_results_{uid}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    target = uid
    
    try:
        async with aiofiles.open(fn, 'w', encoding='utf-8') as f:
            await f.write(f"{'='*70}\n")
            await f.write(f"𝙴𝚗𝚅𝚘𝚇 𝙲𝚑𝚔 - {gate_name} CHECK RESULTS\n")
            await f.write(f"{'='*70}\n\n")
