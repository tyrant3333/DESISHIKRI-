...
from database import (
    ...
    # Deprecated per-user site functions replaced by global ones
    add_global_site, remove_global_site, get_global_sites, get_global_sites_list,
    clear_global_sites, update_global_site_info,
...
MPP_PER_USER_WORKERS = 50     # PayPal mass
SITE_PER_USER_WORKERS = 30
PROXY_PER_USER_WORKERS = 13
BIN_WORKERS = 20

# ── Timeout Configuration (increased to reduce timeouts) ──
...
# ── PER-PROXY Concurrency Limiter (FIX for mass proxy errors) ──
_PROXY_SEMS = {}
...
async def get_proxy_sem(proxy_url: str):
    ...
    async with _PROXY_SEMS_LOCK:
        if proxy_url not in _PROXY_SEMS:
            _PROXY_SEMS[proxy_url] = asyncio.Semaphore(2)   # limit = 2 concurrent requests per proxy
...
ACTIVE_MPP_PROCESSES = {}      # PayPal mass processes
...
# ====================== COMPLETE SITE ERROR DETECTION (unchanged) ======================
SITE_ERROR_KEYWORDS = [
...
async def check_card_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=5, rotator=None, cancel_check=None, http_session=None):
    """Updated with increased retries (5) and improved retry logic for timeouts."""
    if not sites:
    ...
    for attempt in range(max_retries):
        ...
        # If it's a retryable error (including 429), retry after backoff
        ...
        # Permanent result (success or permanent decline)
        if rotator:
...
async def check_rz_with_retry(card, proxies_data=None, user_id=None, max_retries=5, cancel_check=None, http_session=None):
    """Updated with increased retries."""
    tried_proxies = set()
    last = None
    backoff = 1.0
...
async def check_paypal_with_retry(card, proxies_data=None, user_id=None, max_retries=5, cancel_check=None, http_session=None):
    tried_proxies = set()
    last = None
    backoff = 1.0
...
{PE} <b><i>{bs('Sites')}</i></b>
|   {PE} <code>/add</code> ━ <b>{bs('Add sites')}</b> (admin)
|   {PE} <code>/rm</code> ━ <b>{bs('Remove')}</b> (admin)
|   {PE} <code>/sites</code> ━ <b>{bs('View')}</b>
|   {PE} <code>/site</code> ━ <b>{bs('Test all')}</b> (admin)
...
async def info_cmd(event):
    ...
    await ensure_user(event.sender_id)
    plan = await get_user_plan(event.sender_id)
    # sites are now global, not per-user
    sites = await get_global_sites_list()
...
{PE} <b>{bs('Sites')}:</b> <code>{len(sites)}</code> (global)
...
# ====================== SITE MANAGEMENT (NOW GLOBAL, ADMIN ONLY) ======================
...
async def add_site(event):
    ...
    try:
        ...
        existing = await get_global_sites_list()
...
async def list_sites(event):
    ...
    sites_data = await get_global_sites()
    
    if not sites_data: 
        return await styled_reply(event, f"{PE} <b>{bs('No sites')}</b> (admin: /add)", emoji_ids=[CE["warn"]])
    ...
    for i, s in enumerate(sites_data[:30], 1): 
        ...
        text += f"    └ <b>Gateway:</b> <code>{s.get('gateway', 'Unknown')}</code> | <b>Price:</b> <code>{s.get('price', '0')}</code>\n"
...
async def check_sites_cmd(event):
    ...
    await styled_reply(event, f"{PE} <b>{bs('Select Price Range')}</b> {PE}\n{PE} <b>{bs('Sites')}:</b> <code>{len(sites)}</code>\n{PE} <i>{bs('Dead + over-price will be removed from global pool')}</i>", buttons=kb, emoji_ids=[CE["fire"], CE["fire"], CE["globe"], CE["warn"]])
...
async def single_cc_check(event):
    ...
    if not sites: 
        return await styled_reply(event, f"{PE} <b>{bs('No sites!')} </b><code>/add</code> (admin)", emoji_ids=[CE["warn"]])
    ...
    try:
        ...
        async with get_user_sem(uid, "sp"):
            ...
            result, _ = await check_card_with_retry(card, sites, uid, proxies, max_retries=5, rotator=rotator, http_session=http_session)
            bi = await bin_task
...
async def rz_single_check(event):
    ...
    try:
        ...
        bin_task = asyncio.create_task(get_bin_info(card.split('|')[0]))
        result = await check_rz_with_retry(card, proxies, uid, max_retries=5, http_session=http_session)
        bi = await bin_task
        elapsed = round(time.time() - st, 2)
        status = result.get('Status', 'Declined')
        if status in ["Charged", "Approved"]:
...
async def pp_single_check(event):
    ...
    try:
        ...
        bin_task = asyncio.create_task(get_bin_info(card.split('|')[0]))
        result = await check_paypal_with_retry(card, proxies, uid, max_retries=5, http_session=http_session)
        bi = await bin_task
        elapsed = round(time.time() - st, 2)
        status = result.get('Status', 'Declined')
        if status in ["Charged", "Approved"]:
...
async def mass_check_cmd(event):
    ...
    async def shopify_check(card, http_session):
        result, _ = await check_card_with_retry(card, sites, uid, proxies, max_retries=5, rotator=rotator, cancel_check=lambda: ACTIVE_MTXT_PROCESSES.get(uid, {}).get("stopped", True), http_session=http_session)
        return result
...
async def mrz_mass_check_cmd(event):
    ...
    async def rz_check(card, http_session):
        return await check_rz_with_retry(card, proxies, uid, max_retries=5, cancel_check=lambda: ACTIVE_MRZ_PROCESSES.get(uid, {}).get("stopped", True), http_session=http_session)
...
async def mpp_mass_check_cmd(event):
    ...
    async def pp_check(card, http_session):
        return await check_paypal_with_retry(card, proxies, uid, max_retries=5, cancel_check=lambda: ACTIVE_MPP_PROCESSES.get(uid, {}).get("stopped", True), http_session=http_session)
...
async def planall_cmd(event):
    ...
    for tier in PAID_TIERS:
        ...
        if USE_MONGODB and MOTOR_AVAILABLE:
            ...
            if db:
                try:
                    cursor = db.users.find({"plan": tier})
        ...
        for uid, udata in data["users"].items():
            if udata.get("plan") == tier:
                all_users.append({"user_id": int(uid), "plan": udata.get("plan"), "expiry": udata.get("expiry")})
    if not all_users: 
...
async def stats_cmd(event):
    ...
    try:
        ...
        ts2 = await get_total_sites_count()  # now global
