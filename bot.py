...
def classify_response(rj):
    ...
    # If it's a known site error, mark as SiteError so retry can happen
...
async def check_card_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=3, rotator=None, cancel_check=None, http_session=None):
    ...
    # If we got here, all attempts returned SiteError -> treat as Declined
    if last:
        last["Status"] = "Declined"
        return last, -1
    return {"Response": "Max retries", "Price": "-", "Gateway": "-", "Status": "Declined", "card": card}, -1
...
async def check_rz_with_retry(card, proxies_data=None, user_id=None, max_retries=3, cancel_check=None, http_session=None):
    ...
    if last:
        last["Status"] = "Declined"
        return last
    return {"Response": "Max retries", "Price": "-", "Gateway": "RazorPay", "Status": "Declined", "card": card}
    ...
    async def worker(card):
        ...
        async with user_sem:
            ...
            try:
                ...
                # Count as Error only if status is "Error" (which we now avoid for SiteError)
                if status == "Error": 
                    errors += 1
                    ...
                    # Treat anything else as Declined (including SiteError turned Declined)
                    declined += 1
