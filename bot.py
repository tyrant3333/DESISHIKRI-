# bot.py – Full updated with global sites (admin only) + improved rotation
...
from database import (
    ...
    # site functions replaced with global ones
...
# ====================== SMART ROTATION ENGINE (UPDATED WITH GLOBAL USER INDEX) ======================
# Global rotation state per user
_USER_SITE_IDX = {}
_USER_SITE_HASH = {}          # hash of the sorted site list to detect changes
...
class SmartRotator:
    ...
    def _get_site_hash(self, sites):
        """Return a hash of the sorted site list to detect changes."""
        return hash(tuple(sorted(sites)))

    def pick_site(self, sites, exclude=None):
        if not sites:
            return None
        exclude = exclude or set()
        # If we have a user_id, use global index and handle list changes
        if self.user_id is not None:
            current_hash = self._get_site_hash(sites)
            old_hash = _USER_SITE_HASH.get(self.user_id)
            if old_hash != current_hash:
                # Site list changed – reset index
                _USER_SITE_IDX[self.user_id] = 0
                _USER_SITE_HASH[self.user_id] = current_hash
            idx = _USER_SITE_IDX.get(self.user_id, 0)
            # Filter available sites (alive and not excluded)
            ...
            if not available:
                return None
            # Pick from available using global index
            chosen = available[idx % len(available)]
            # Advance global index for next call
            _USER_SITE_IDX[self.user_id] = (idx + 1) % len(available)
            return chosen
        else:
            # Fallback (no user) – local round‑robin
            available = [s for s in sites if s not in exclude and self._site_fails.get(s, 0) < 5]
            if not available:
                available = [s for s in sites if s not in exclude]
            if not available:
            ...
            if not available:
                return None
            # Use local index
            self._site_idx = getattr(self, '_site_idx', 0) % len(available)
            chosen = available[self._site_idx]
...
# FULL UPDATED SITE_ERROR_KEYWORDS WITH ALL DEAD RESPONSES
SITE_ERROR_KEYWORDS = [
    # === AAPKE PHOTO WALE ERRORS (UPPERCASE DIRECT) ===
    ...
    # === NEW SHOPIFY ERRORS (ADDED) ===
    'MERCHANDISE_LINE_ITEM_NOT_PURCHASABLE',
    'merchandise_line_item_not_purchasable',
    'CHECKOUT_CART_LINE_ITEM_LIMIT_REACHED',
    'checkout_cart_line_item_limit_reached',
    # 'TOKENIZATION_FAILED',  # removed from site errors, moved to declined
    '400',
    ...
    # ===== ADDED KEYWORDS FOR BETTER QUALITY CONTROL =====
    'no_shopify_payments_gateway',
    'cart_failed',
    # 'no_product',  # moved to declined
    # 'tokenization_failed',  # moved to declined
    'site_requires_login',
]
...
def is_site_error(text):
    ...
    # Specific decline patterns that should not be treated as site errors
    decline_specific = [
        'tokenization_failed', 'no_product', 'cart_failed',
        'no_shopify_payments_gateway', 'site_requires_login',
        'too many values to unpack'  # ADDED
    ]
    if any(ds in lower for ds in decline_specific):
        return False  # These are not site errors, they will be classified as Declined later
...
# ====================== UPDATED is_site_truly_dead FUNCTION ======================
def is_site_truly_dead(response, price):
    if not response:
        return True
    
    # FIRST: Direct uppercase check for exact matches
    response_upper = response.upper().strip()
    
    exact_dead = [
        # NEW KEYWORDS FROM AAPKE PHOTO
        'MERCHANDISE_PRODUCT_NOT_PUBLISHED_IN_BUYER_LOCATION',
        'WAITING_PENDING_TERMS',
        'INVENTORYRESERVATIONFAILURE',
        'DELIVERY_DELIVERY_LINE_DETAIL_CHANGED',
...
def classify_response(rj):
    ...
    # First, check for specific decline patterns that should NOT be treated as site errors
    decline_specific = [
        'tokenization_failed', 'no_product', 'cart_failed',
        'no_shopify_payments_gateway', 'site_requires_login',
        'no_payment_method', 'payment method not available',
        'too many values to unpack'  # ADDED
    ]
    if any(ds in rl for ds in decline_specific):
        return {"Response": ar, "Price": price, "Gateway": gw, "Status": "Declined"}
    
    # Then check for site errors (connection issues, timeouts, etc.)
    ...
    try:
        ...
        async with s.get(url) as r:
            ...
            try: 
                ...
                # If JSON decode fails, check raw text for known decline patterns
                try:
                    raw_text = await r.text()
                except:
                    raw_text = ""
                raw_lower = raw_text.lower()
                decline_patterns = ['too many values to unpack', 'tokenization_failed', 'no_product', 'cart_failed', 'no_shopify_payments_gateway', 'site_requires_login']
                if any(p in raw_lower for p in decline_patterns):
                    return {"Response": raw_text[:100], "Price": "-", "Gateway": "Unknown", "Status": "Declined", "card": card, "site": site}
                else:
...
async def check_card_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=3, rotator=None, cancel_check=None, http_session=None):
    ...
    if last:
        # If the last error was SiteError, treat as Declined to avoid errors
        if last.get("Status") == "SiteError":
    ...
    try:
        ...
        async with s.get(url) as resp:
            ...
            try: 
                ...
                if 'too many values to unpack' in raw_text.lower():
                    return {'site': site, 'status': 'dead', 'price': '-', 'gateway': '-', 'response': raw_text[:100]}
                else:
...
async def check_rz_with_retry(card, proxies_data=None, user_id=None, max_retries=3, cancel_check=None, http_session=None):
    ...
    if last:
        # If RetryError, treat as Declined to avoid errors
        if last.get("Status") == "RetryError":
            last["Status"] = "Declined"
        else:
            last["Status"] = "Error"
        return last
...
async def _build_status_text():
    ...
    if sys_info.get("error"):
        return f"⌬ <b>𝐄𝐫𝐫𝐨𝐫</b> ↬ <code>❌ {sys_info['error']}</code>\n⌬ <b>𝐁𝐨𝐭 𝐁𝐲</b> ↬ <a href='https://t.me/Tyrant_Xd_Rebot'>𝑻𝒚𝒓𝒂𝒏𝒕</a>"
    s = sys_info
    msg = (
...
{PE} <b><i>{bs('Sites')}</i></b> ({bs('Admin only')})
...
async def info_cmd(event):
    ...
    await ensure_user(event.sender_id)
    plan = await get_user_plan(event.sender_id)
    global_sites = await get_global_sites_with_info()  # show global sites
...
# ====================== SITE MANAGEMENT (ADMIN ONLY, GLOBAL) ======================
...
async def single_cc_check(event):
    ...
    # Use global sites for everyone
    sites = await get_global_sites()
    proxies = await get_all_user_proxies(uid)
    if not sites: 
        return await styled_reply(event, f"{PE} <b>{bs('No global sites!')} </b><code>/add</code>", emoji_ids=[CE["warn"]])
    ...
    st = time.time()
    rotator = SmartRotator(user_id=uid)   # persistent per‑user rotation
    try:
...
# ====================== UPDATED GENERIC MASS PROCESSOR WITH DETAILED FILE ======================
    ...
    async def worker(card):
        ...
        async with user_sem:
            if is_stopped(): 
                return
            try:
                # Use reduced retries for mass (2) to speed up
                if is_rz:
                    result = await check_func(card, http_session)  # check_func already has retry logic
                else:
                    # Shopify mass uses check_card_with_retry with max_retries=2
                    result, _ = await check_func(card, http_session)  # check_func returns (result, index)
                if is_stopped(): 
                    return
                status = result.get("Status", "Declined")
...
async def mass_check_cmd(event):
    ...
    async def shopify_check(card, http_session):
        # reduce retries to 2 for mass
        result, _ = await check_card_with_retry(card, sites, uid, proxies, 2, rotator, cancel_check=lambda: ACTIVE_MTXT_PROCESSES.get(uid, {}).get("stopped", True), http_session=http_session)
        return result
...
async def chk_pref_cb(event):
    ...
    rotator = data.get("rotator", SmartRotator(user_id=uid))
...
async def mrz_mass_check_cmd(event):
    ...
    async def rz_check(card, http_session):
        # reduce retries to 2 for mass
        return await check_rz_with_retry(card, proxies, uid, max_retries=2, cancel_check=lambda: ACTIVE_MRZ_PROCESSES.get(uid, {}).get("stopped", True), http_session=http_session)
