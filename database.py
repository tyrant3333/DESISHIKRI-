```python
# database.py - Full updated with global site support
    ...
    try:
        if os.path.exists(db_file):
            ...
            data = {
                ...
                "cards": [],
                "proxies": {},
                "sites": {},        # per-user sites (legacy)
                "global_sites": {},  # new global sites storage
                "plan_codes": {}
            }
...
async def add_global_site(site: str, gateway: str = "Unknown", price: str = "0") -> bool:
    ...
    if "global_sites" not in data:
        data["global_sites"] = {}
    data["global_sites"][site] = {"gateway": gateway, "price": price}
    _save_json_db(data)
    return True

async def get_global_sites() -> List[str]:
    """Get all global sites (list of site strings)"""
    if USE_MONGODB and MOTOR_AVAILABLE:
        db = await get_mongo_db()
        if db:
            try:
                doc = await db.global_sites.find_one({"_id": "global_sites"})
    ...
    data = _load_json_db()
    sites_dict = data.get("global_sites", {})
    return list(sites_dict.keys())

async def get_global_sites_with_info() -> List[Dict]:
    """Get global sites with info (gateway, price)"""
    if USE_MONGODB and MOTOR_AVAILABLE:
        db = await get_mongo_db()
        if db:
            try:
...
async def remove_global_site(site: str) -> bool:
    ...
    data = _load_json_db()
    if site in data.get("global_sites", {}):
        del data["global_sites"][site]
        _save_json_db(data)
        return True
    return False

# ============ PER-USER SITE FUNCTIONS (legacy, kept for compatibility) ============

async def add_site_db(user_id: int, site: str, gateway: str = "Unknown", price: str = "0") -> bool:
    """Add site (per-user) - legacy, not used in new bot"""
    if USE_MONGODB and MOTOR_AVAILABLE:
        db = await get_mongo_db()
        if db:
            try:
...
async def get_user_sites_with_info(user_id: int) -> List[Dict]:
    """Get user sites with info - legacy"""
    if USE_MONGODB and MOTOR_AVAILABLE:
        db = await get_mongo_db()
        if db:
            try:
    ...
    if USE_MONGODB and MOTOR_AVAILABLE:
        ...
        if db:
            try:
                ...
                # Import PLANS from bot (circular import avoided by using a local copy or dynamic)
                # We'll define a simple mapping or use the global PLANS from bot if available.
                # To avoid circular import, we'll use a hardcoded mapping or import inside function.
                # We'll define a local PLANS mapping for this function.
                local_plans = {
                    "trial": {"duration_days": 2, "tier": "Trial"},
                    "plan1": {"duration_days": 7, "tier": "Core"},
                    "plan2": {"duration_days": 15, "tier": "Elite"},
                    "plan3": {"duration_days": 30, "tier": "Root"},
                    "plan4": {"duration_days": 90, "tier": "X"}
                }
                if plan_key not in local_plans:
                    return False, "invalid"
                
                plan_info = local_plans[plan_key]
    ...
    if USE_MONGODB and MOTOR_AVAILABLE and MONGODB_URI:
        try:
            ...
            if db:
                ...
                # Ensure global_sites collection exists
                await db.global_sites.create_index("_id")
                print("✅ MongoDB ready!")
        except Exception as e:
...
# ============ GLOBAL SITES (Legacy alias) ============
# We already have the proper functions above, but keep these for backward compatibility
async def add_global_site_alias(site: str) -> bool:
    return await add_global_site(site, "Unknown", "0")

async def get_global_sites_alias() -> List[str]:
    return await get_global_sites()

async def remove_global_site_alias(site: str) -> bool:
