# database.py - Completely Fixed (No Import Errors)
import os
import datetime
import random
import string
    ...
    try:
        if os.path.exists(db_file):
            ...
            data = {
                ...
                "sites": {},
                "plan_codes": {},
                "global_sites": []   # NEW: global site list
            }
...
async def init_db():
    """Initialize database - creates JSON file or connects to MongoDB"""
    print("=" * 50)
...
async def add_global_site(site: str, gateway: str = "Unknown", price: str = "0") -> bool:
    """Add a site to global pool (admin only). Avoid duplicates."""
    ...
    if USE_MONGODB and MOTOR_AVAILABLE:
        ...
        if db:
            try:
                await db.global_sites.update_one(
                    {"_id": "global"},
                    {"$push": {"sites": {"site": site, "gateway": gateway, "price": price}}},
                    upsert=True
                )
                return True
            except:
    ...
    data = _load_json_db()
    if "global_sites" not in data:
        data["global_sites"] = []
    data["global_sites"].append({"site": site, "gateway": gateway, "price": price})
    _save_json_db(data)
    return True

async def remove_global_site(site: str) -> bool:
    """Remove a site from global pool."""
    site = site.strip().lower()
    # normalize
    if site.startswith('http://') or site.startswith('https://'):
        from urllib.parse import urlparse
    ...
    if USE_MONGODB and MOTOR_AVAILABLE:
        ...
        if db:
            try:
                result = await db.global_sites.update_one(
                    {"_id": "global"},
                    {"$pull": {"sites": {"site": site}}}
                )
    ...
    data = _load_json_db()
    original_len = len(data.get("global_sites", []))
    data["global_sites"] = [s for s in data.get("global_sites", []) if s.get("site") != site]
    if len(data["global_sites"]) < original_len:
        _save_json_db(data)
        return True
    return False

async def get_global_sites() -> List[Dict]:
    """Get all global sites with info."""
    if USE_MONGODB and MOTOR_AVAILABLE:
        db = await get_mongo_db()
        if db:
            try:
                doc = await db.global_sites.find_one({"_id": "global"})
    ...
    data = _load_json_db()
    return data.get("global_sites", [])

async def get_global_sites_list() -> List[str]:
    """Get list of site strings only."""
    sites = await get_global_sites()
    return [s.get('site') for s in sites if s.get('site')]

async def clear_global_sites() -> int:
    """Clear all global sites. Returns count removed."""
    sites = await get_global_sites()
    count = len(sites)
...
async def update_global_site_info(site: str, gateway: str, price: str) -> bool:
    """Update gateway/price for a global site."""
    site = site.strip().lower()
    if site.startswith('http://') or site.startswith('https://'):
        from urllib.parse import urlparse
        parsed = urlparse(site)
    ...
    if USE_MONGODB and MOTOR_AVAILABLE:
        ...
        if db:
            try:
                await db.global_sites.update_one(
                    {"_id": "global", "sites.site": site},
                    {"$set": {"sites.$.gateway": gateway, "sites.$.price": price}}
                )
                return True
            except:
                pass
    ...
    data = _load_json_db()
    for s in data.get("global_sites", []):
        if s.get('site') == site:
            s['gateway'] = gateway
            s['price'] = price
            _save_json_db(data)
...
async def update_site_info(user_id: int, site: str, gateway: str, price: str) -> bool:
    """Deprecated: use global sites."""
    return await update_global_site_info(site, gateway, price)
...
async def get_users_with_sites() -> int:
    """Deprecated: return 1 if global sites exist."""
    sites = await get_global_sites()
    return 1 if sites else 0

async def get_sites_per_user() -> List[Dict]:
    """Deprecated: return global site count."""
    sites = await get_global_sites()
    return [{"user_id": 0, "cnt": len(sites)}]

async def get_all_sites_detail() -> List[Dict]:
    """Get all global sites with detail."""
    sites = await get_global_sites()
    result = []
    for s in sites:
        result.append({
...
# ============ GLOBAL SITES (Legacy) - kept for compatibility ============

async def add_global_site_legacy(site: str) -> bool:
    return await add_global_site(site)

async def get_global_sites_legacy() -> List[str]:
    return await get_global_sites_list()

async def remove_global_site_legacy(site: str) -> bool:
    return await remove_global_site(site)
