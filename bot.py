# bot.py – Full updated with global sites (admin only) + improved rotation + FULL JSON error detection
...
def classify_response(rj):
    ...
    # 🔥 NEW: Also scan the entire JSON for site-error keywords
    raw_str = json.dumps(rj).lower()
    if any(kw in raw_str for kw in SITE_ERROR_KEYWORDS):
...
async def check_card_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=3, rotator=None, cancel_check=None, http_session=None):
    if not sites:
        return {"Response": "No sites", "Price": "-", "Gateway": "-", "Status": "Declined", "card": card}, -1
    tried_sites = set()
    tried_proxies = set()
    last = None
    for attempt in range(max_retries):
        if cancel_check and cancel_check():
            return {"Response": "Stopped", "Price": "-", "Gateway": "-", "Status": "Declined", "card": card}, -1
        if rotator: 
    ...
    # All retries failed with SiteError – classify as Error (not Declined)
    if last:
        last["Status"] = "Error"
        return last, -1
...
# ====================== UPDATED test_site – SCANS ENTIRE JSON ======================
async def test_site(site, proxy_data=None, http_session=None):
    ...
    try:
        ...
        # 🔥 CRITICAL FIX: Scan the ENTIRE JSON response for any site-error keyword
        raw_str = json.dumps(raw).lower()
        if any(kw in raw_str for kw in SITE_ERROR_KEYWORDS):
            return {'site': site, 'status': 'dead', 'price': price, 'gateway': gateway, 'response': rm}
        
        if rm and rm.upper().startswith(('SITE_', 'NO_', 'CHECKOUT_', 'MERCHANDISE_', 'TOKENIZATION_', 'CART_')):
            return {'site': site, 'status': 'dead', 'price': price, 'gateway': gateway, 'response': rm}
        if rm and "PRICE_OVER_MAX" in rm.upper():
            return {'site': site, 'status': 'dead', 'price': price, 'gateway': gateway, 'response': rm}
...
async def check_rz_with_retry(card, proxies_data=None, user_id=None, max_retries=3, cancel_check=None, http_session=None):
    ...
    for attempt in range(max_retries):
        if cancel_check and cancel_check():
            return {"Response": "Stopped", "Price": "-", "Gateway": "RazorPay", "Status": "Declined", "card": card}
        proxy_data = None
        if proxies_data:
            available_px = [p for p in proxies_data if p.get('proxy_url') not in tried_proxies] or list(proxies_data)
            proxy_data = random.choice(available_px)
    ...
    if last:
        # For RazorPay, retry errors are also treated as Errors
        last["Status"] = "Error"
        return last
...
# ====================== CLIENT ======================
client = TelegramClient('EnVoXChkBoT', API_ID, API_HASH)
    ...
    async def worker(card):
        ...
        async with user_sem:
            ...
            try:
                ...
                # Now Error is a real status from check_func (after retries)
                if status == "Error": 
                    errors += 1
                    ...
                    # Anything else is Declined (including fallback)
                    declined += 1
...
async def send_detailed_results_file(uid, charged, approved, declined, errors, total, detailed_results, gate_name):
    ...
    fn = f"𝙴𝚗𝚅𝚘𝚇_𝙲𝚑𝚔_{uid}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    target = uid
