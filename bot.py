...
# ========== GLOBAL SEMAPHORE FOR SHOPIFY API (to reduce 429) ==========
_SHOPIFY_API_SEM = asyncio.Semaphore(25)
...
async def check_card_api(card, site, proxy_data=None, user_id=None, http_session=None):
    ...
    try:
        ...
        async with _SHOPIFY_API_SEM:  # global semaphore to limit 429
...
async def check_card_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=7, rotator=None, cancel_check=None, http_session=None):
    if not sites:
    ...
    last = None
    site_attempt = 0
    backoff = 3.0  # start at 3 seconds
...
async def check_paypal_api(card, proxy_data=None, user_id=None, http_session=None):
    ...
    try:
        ...
        async with s.get(url) as r:
            ...
            if r.status == 429:
                return {"Response": "HTTP_429 - Too Many Requests", "Price": "$5.00", "Gateway": "PayPal", "Status": "RetryError", "card": card}
            if r.status != 200:
                return {"Response": f"HTTP_{r.status}: {response_text[:100]}", "Price": "$5.00", "Gateway": "PayPal", "Status": "RetryError", "card": card}
            try:
...
async def info_cmd(event):
    ...
    try:
        ...
        if db:
            user_doc = await db.users.find_one({"_id": str(event.sender_id)})
            if user_doc:
                expiry = user_doc.get("expiry")
        else:
            db_data = _load_json_db()
            user_data = db_data["users"].get(str(event.sender_id), {})
            expiry = user_data.get("expiry")
    except:
        db_data = _load_json_db()
        user_data = db_data["users"].get(str(event.sender_id), {})
...
async def single_cc_check(event):
    ...
    try:
        ...
        async with get_user_sem(uid, "sp"):
            ...
            result, _ = await check_card_with_retry(card, sites, uid, proxies, max_retries=7, rotator=rotator, http_session=http_session)
            bi = await bin_task
...
async def mass_check_cmd(event):
    ...
    async def shopify_check(card, http_session):
        result, _ = await check_card_with_retry(card, sites, uid, proxies, max_retries=7, rotator=rotator, cancel_check=lambda: ACTIVE_MTXT_PROCESSES.get(uid, {}).get("stopped", True), http_session=http_session)
        return result
