...
# ====================== IMPROVED CHECK CARD WITH RETRY (returns a dict, no tuple) ======================
async def check_card_with_retry(card, sites, user_id=None, max_retries=4, cancel_check=None, http_session=None):
    """
    Try up to max_retries random sites. If a site returns dead, try the next.
    Returns the first non-dead result dict, or the last dead result dict.
    The result dict includes 'site' field.
    """
    if not sites:
        return {"Response": "No sites available", "Price": "-", "Gateway": "-", "Status": "Error", "card": card, "site": "N/A"}
    ...
    for attempt in range(max_retries):
        if cancel_check and cancel_check():
            return {"Response": "Stopped", "Price": "-", "Gateway": "-", "Status": "Error", "card": card, "site": "N/A"}
    ...
    if last_result:
        last_result["site"] = last_site if last_site else "N/A"
        return last_result
    else:
        return {"Response": "All sites failed", "Price": "-", "Gateway": "-", "Status": "Error", "card": card, "site": "N/A"}
...
async def single_cc_check(event):
    ...
    try:
        ...
        async with get_user_sem(uid, "sp"):
            ...
            result = await check_card_with_retry(card, sites, uid, max_retries=4, http_session=http_session)
            bi = await bin_task
...
# ====================== /msp (Shopify Mass) - UI unchanged, uses improved retry ======================
...
async def mass_check_cmd(event):
    ...
    async def shopify_check(card, http_session):
        # Use improved retry with dead-site detection
        return await check_card_with_retry(
            card, sites, uid, max_retries=4,
    ...
    if from_inline:
        ...
        USER_APPROVED_PREF[f"chk_{uid}"] = {"cards": cards, "sites": sites, "proxies": proxies, "event": event, "pref_msg": pm, "uid": uid}
...
# ====================== GENERIC MASS PROCESSOR (unchanged UI) ======================
