...
# FIX: HTTP_400 retry, separate SiteError counter, detailed file report
...
def is_card_retry_needed(response_text):
    ...
    # ADDED: Treat HTTP 400 as retryable (often temporary)
    if 'http_400' in lower:
        return True
...
async def check_card_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=3, rotator=None, cancel_check=None, http_session=None):
    ...
    for attempt in range(max_retries):
        ...
        # --- PER-PROXY CONCURRENCY LIMITER (FIX) ---
        if proxy_data:
...
def build_rz_api_url(cc, proxy_data=None, site=None):
    # New RazorPay API requires a site parameter. Use a default if not provided.
    if not site:
        site = "https://razorpay.me/@tpstech"   # default from example
...
async def check_rz_api(card, proxy_data=None, user_id=None, http_session=None):
    uid = user_id or "?"
    try:
        # Use a fixed site for RazorPay (can be changed by admin if needed)
...
def build_paypal_api_url(cc, proxy_data=None):
    """Build PayPal API URL with card and optional proxy"""
...
def classify_paypal_response(response_text):
    """Classify PayPal API response as Charged, Approved, Declined, or Error"""
    gate = 'PayPal'
    ...
    # Approved keywords - needs additional authentication
    approved_keywords = [
    ...
    # Declined keywords - permanent failure
    declined_keywords = [
...
async def check_paypal_api(card, proxy_data=None, user_id=None, http_session=None):
    """Check single card with PayPal gateway"""
    uid = user_id or "?"
    try:
...
async def check_paypal_with_retry(card, proxies_data=None, user_id=None, max_retries=3, cancel_check=None, http_session=None):
    ...
    for attempt in range(max_retries):
        ...
        # Apply per-proxy concurrency limiter
        if proxy_data:
            proxy_url = proxy_data.get('proxy_url')
            sem = await get_proxy_sem(proxy_url)
            async with sem:
...
async def _run_mass_process(event, cards, proxies, send_approved, process_store, stop_prefix, check_func, gate_name, sem_type):
    ...
    try: 
        ...
        username, name = f"user_{uid}", "User"
    total = len(cards)
    checked = charged = approved = declined = errors = site_errors = 0   # ADDED site_errors
    ...
    async def worker(card):
        nonlocal checked, charged, approved, declined, errors, site_errors, lcd, lrd
        if is_stopped(): 
            return
        async with user_sem:
            if is_stopped(): 
    ...
    # Updated final text with Site Errors
    ft = f"""{PE} <b>{bs('Complete')}{stop_label}</b> {PE}\n<b>━━━━━━━━━━━━━━━━━</b>\n{PE} <b>{bs('Charged')}</b> ━ <code>{charged}</code>\n{PE} <b>{bs('Approved')}</b> ━ <code>{approved}</code>\n{PE} <b>{bs('Declined')}</b> ━ <code>{declined}</code>\n{PE} <b>{bs('Site Errors')}</b> ━ <code>{site_errors}</code>\n{PE} <b>{bs('Other Errors')}</b> ━ <code>{errors}</code>\n<b>━━━━━━━━━━━━━━━━━</b>\n{PE} <b>{bs('Checked')}</b> ━ <code>{checked}/{total}</code>"""
    fkb = [
        [pbtn(f"{bs('C')} ━ {charged}", "none"), pbtn(f"{bs('A')} ━ {approved}", "none")],
        [pbtn(f"{bs('D')} ━ {declined}", "none"), pbtn(f"{bs('SE')} ━ {site_errors}", "none")],
        [pbtn(f"{bs('E')} ━ {errors}", "none"), pbtn(f"{bs('T')} ━ {checked}/{total}", "none")],
        [pbtn(f"{h}{bs('h')}{m}{bs('m')}{s}{bs('s')}", "none")]
    ]
    for _ in range(3):
        try: 
    ...
    await send_detailed_results_file(uid, charged, approved, declined, errors, site_errors, total, detailed_results, gate_name)
...
async def send_detailed_results_file(uid, charged, approved, declined, errors, site_errors, total, detailed_results, gate_name):
    """Send a detailed file with card results including site, response, gateway, price, and separate Site Errors"""
    ...
    try:
        async with aiofiles.open(fn, 'w', encoding='utf-8') as f:
            ...
            await f.write(f"⚠️ SITE ERRORS: {site_errors}\n")
            await f.write(f"⚠️ OTHER ERRORS: {errors}\n")
            await f.write(f"\n{'='*70}\n\n")
            ...
            # Site Errors
            if site_errors > 0:
                await f.write(f"\n⚠️ SITE ERROR CARDS ({site_errors})\n")
                await f.write(f"{'-'*50}\n")
                for r in detailed_results:
                    if r.get("result_type") == "SITE_ERROR":
                        await f.write(f"Card: {r['card']}\n")
                        await f.write(f"  Status: {r['status']}\n")
                        await f.write(f"  Gateway: {r['gateway']}\n")
                        await f.write(f"  Response: {r['response']}\n")
