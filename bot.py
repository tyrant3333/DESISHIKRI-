# bot.py – Full updated with global sites (admin only) + improved rotation + speed optimizations
...
async def get_user_http_session(uid, purpose="general"):
    ...
    if session is None or session.closed:
        # Use shorter timeout for Shopify single checks
...
async def check_card_api(card, site, proxy_data=None, user_id=None, http_session=None):
    ...
    try:
        ...
        # Use a per-request timeout to avoid hanging too long
...
async def test_site(site, proxy_data=None, http_session=None):
    ...
    try:
        ...
        # Check additional error keywords in response
...
async def single_cc_check(event):
    ...
    try:
        ...
        async with get_user_sem(uid, "sp"):
            ...
            # Use max_retries=2 for faster single check
...
async def rz_single_check(event):
    ...
    try:
        ...
        bin_task = asyncio.create_task(get_bin_info(card.split('|')[0]))
        # RazorPay also use max_retries=2 for speed
