# bot.py – Full original code with 4xx abort fix (timeouts reverted to 60s)
...
async def check_card_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=3, rotator=None, cancel_check=None, http_session=None):
    ...
    for attempt in range(max_retries):
        ...
        # === FIX: If we get a 4xx client error, do not retry – return immediately ===
        if result.get("Status") == "SiteError" and result.get("Response", "").startswith("HTTP_4"):
            if rotator:
