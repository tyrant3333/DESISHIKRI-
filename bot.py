...
# ====================== PAYPAL API (FIXED WITH CORRECT RESPONSE PARSING) ======================
...
def classify_pp_response(response_data):
    ...
    full_response = f"{status_text} | {response_msg}" if status_text and response_msg else response_msg or status_text
    
    # Check for Charged (success true and status contains charged/success)
    if success == 'true':
        # Check if status indicates charged
        if any(word in status_text.lower() for word in ['charged', 'success', 'approved', 'live']):
            return {"Response": full_response[:200], "Gateway": gate, "Status": "Charged", "amount": amount}
        else:
            return {"Response": full_response[:200], "Gateway": gate, "Status": "Approved", "amount": amount}
    
    # Check for Declined
    declined_keywords = [
        'declined', 'invalid', 'failed', 'error', 'cancelled', 'card_declined',
        'insufficient', 'expired', 'stolen', 'lost', 'restricted', 'fraud'
    ]
    
    lower_full = (status_text + " " + response_msg).lower()
    
    for kw in declined_keywords:
        if kw in lower_full:
            return {"Response": full_response[:200], "Gateway": gate, "Status": "Declined", "amount": amount}
    
    # Check for Error (retryable)
    error_keywords = [
        'timeout', 'connection', 'server', 'unavailable', 'rate limit',
        'too many', 'proxy', 'http_500', 'http_502', 'http_503', 'http_504'
    ]
    
    for kw in error_keywords:
        if kw in lower_full:
            return {"Response": full_response[:200], "Gateway": gate, "Status": "Error", "amount": amount}
...
async def check_pp_api(card, site_url, amount="5.00", proxy_data=None, user_id=None, http_session=None):
    ...
    try:
        ...
        async with s.get(url, timeout=aiohttp.ClientTimeout(total=PP_TIMEOUT)) as r:
            if r.status == 403:
                return {"Response": f"HTTP_403: Site may be blocked or invalid", "Gateway": "PayPal", "Status": "Error", "card": card, "site": site_url, "amount": amount}
            elif r.status != 200:
...
async def pp_single_check(event):
    ...
    if len(parts) >= 2:
        ...
        if is_valid_url_or_domain(potential_site):
            ...
            # Try to extract card from first param if not a site
