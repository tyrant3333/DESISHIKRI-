...
from database import (
    ...
    _load_json_db, get_mongo_db, USE_MONGODB, MOTOR_AVAILABLE
)
...
API_TIMEOUT = 90               # increased from 60 to reduce timeouts
BIN_TIMEOUT = 60
PROXY_TIMEOUT = 20
RZ_TIMEOUT = 60
PP_TIMEOUT = 60
...
async def check_card_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=5, rotator=None, cancel_check=None, http_session=None):
    """Updated with smart retry - retries same card for temporary errors up to 5 times with exponential backoff"""
    if not sites:
...
{PE} <b><i>{bs('Sites')}</i></b> (admin only)
